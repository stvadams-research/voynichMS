...
| C2 | Hardcoded Model Sensitivity Weights | 2 | DONE | Moved weights to `configs/phase6_functional/model_params.json`, added `get_model_params()` to config.py with proper `json`/`pathlib` imports. Models load from config with hardcoded fallbacks. `evaluation.py` now also loads dimension weights from config. Rationales documented in `governance/CALIBRATION_NOTES.md`. |

## High-Severity Issues (H2 Series) - COMPLETED

| ID | Issue | Workstream | Status | Notes |
|---|---|---|---|---|
| **H1** | Boolean Truthiness Bug in Mapping Stability | Logic | DONE | Replaced truthiness check with explicit `is not None` checks at lines 116, 337, 363. Added 10 unit tests in `tests/phase2_analysis/test_mapping_stability.py` covering zero-value propagation across all explanation classes. |
...
| X2 | Shadowing `__init__.py` in `tests/phase2_analysis/` | DONE | Removed `tests/phase2_analysis/__init__.py` which shadowed `src/phase2_analysis/` and prevented test imports. |
