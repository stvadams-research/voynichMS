...
**Methodology:** Per `planning/core_audit/CODE_AUDIT_AND_CLEANUP_PLAYBOOK.md`
...
| CLI entry point | 1 | `src/phase1_foundation/cli/main.py` |
| Shell scripts | 2 | `scripts/ci_check.sh`, `scripts/verify_reproduction.sh` |
| Audit scripts | 2 | `scripts/core_audit/` |
| Notebooks | 0 | None |
...
**Configuration files:** `configs/phase6_functional/model_params.json` (centralized model parameters).
...
| ID | Severity | Finding | Location |
|----|----------|---------|----------|
| R1 | Critical | `random.choice/randint/random` without seed | `phase5_mechanism/generators/pool_generator.py:27-31` |
| R2 | Critical | 7 `random.uniform()` in fallback paths, unseeded | `phase3_synthesis/refinement/feature_discovery.py` |
| R3 | Critical | `random.seed(seed)` then 8+ bare `random.*()` calls | `phase1_foundation/controls/self_citation.py:19-48` |
...
**RandomnessController adoption:** EXISTS at `phase1_foundation/core/randomness.py` but is NOT used by any file in `phase3_synthesis/`, `phase5_mechanism/`, or `phase1_foundation/controls/`.
...
| IO3 | Medium | 8+ public methods lack return type hints | `phase3_synthesis/text_generator.py`, `alignment/engine.py`, `refinement/feature_discovery.py` |
...
|--------|-----------|--------|
| `phase1_foundation/metrics/` | `test_boundary_cases.py` | Partial |
| `phase1_foundation/core/geometry` | `test_geometry.py` | Partial |
| `phase1_foundation/core/ids` | `test_ids.py` | Partial (1 failing) |
| `phase2_analysis/stress_tests/` | `test_mapping_stability.py` | Minimal (new) |
| `core_audit/` | `test_determinism.py` | Minimal |
| `integration/` | `test_enforcement.py` | Minimal |
| **phase3_synthesis/** | **None** | **NOT TESTED** |
| **phase5_mechanism/** | **None** | **NOT TESTED** |
| **phase4_inference/** | **None** | **NOT TESTED** |
| **phase6_functional/** | **None** | **NOT TESTED** |
| **phase7_human/** | **None** | **NOT TESTED** |
| **phase8_comparative/** | **None** | **NOT TESTED** |
...
| TD2 | Medium | "Page" vs "Folio" confused: PageRecord stores folio strings like "f1r" | `core/ids.py`, `phase7_human/quire_analysis.py:23`, `phase3_synthesis/interface.py:74` |
...
| LG2 | Medium | 2 `print()` statements in non-CLI code | `qc/anomalies.py:27`, `phase2_analysis/sensitivity.py:36` |
...
| `governance/governance/METHODS_REFERENCE.md` | Good | Dual formula issue not documented; no confounds section |
| `governance/CONFIG_REFERENCE.md` | Good | No randomness control documented; rationale incomplete |
| `governance/governance/REPRODUCIBILITY.md` | Moderate | Only covers Phase 2-3; Phase 4-5 missing entirely |
| `governance/CALIBRATION_NOTES.md` | Excellent | Comprehensive rationale for thresholds |
| `governance/SENSITIVITY_ANALYSIS.md` | Poor | Plan exists but NEVER EXECUTED; no results |
| `governance/GLOSSARY.md` | Good | Core terms defined |
| `governance/RUNBOOK.md` | Moderate | Phase 2 section incomplete; Phase 4-6 missing |
| `governance/FEATURE_RANGES.md` | Good | 16 features documented with ranges |
...
| Phase 2 (Analysis) | Yes | Documented in governance/REPRODUCIBILITY.md |
| Phase 3 (Synthesis) | Partial | `grammar_based.py` now seeded; but `feature_discovery.py` has 7 unseeded random calls |
| Phase 4-5 (Mechanism) | No | Not documented in governance/REPRODUCIBILITY.md; simulators use local seeds but no orchestration guide |
...
|----|----------|----------|---------|----------|
| C1 | Critical | Randomness | Unseeded `random.*()` in pool_generator | `phase5_mechanism/generators/pool_generator.py:27-31` |
| C2 | Critical | Randomness | 7 unseeded `random.uniform()` in fallback paths | `phase3_synthesis/refinement/feature_discovery.py` |
| C3 | Critical | Randomness | Global `random.seed()` then bare calls | `phase1_foundation/controls/self_citation.py:19-48` |
| C4 | Critical | Randomness | Global `random.seed()` then bare calls | `phase1_foundation/controls/synthetic.py:13` |
| C5 | Critical | Circularity | `BASELINE_INFO_DENSITY=4.0` embeds observed Voynich value | `phase2_analysis/anomaly/stability_analysis.py:40` |
| C6 | Critical | Circularity | `BASELINE_LOCALITY=3.0` embeds observed Voynich value | `phase2_analysis/anomaly/stability_analysis.py:41` |
| C7 | Critical | Circularity | `BASELINE_ROBUSTNESS=0.70` embeds observed Voynich value | `phase2_analysis/anomaly/stability_analysis.py:42` |
...
| H5 | High | Randomness | `random.Random(seed)` bypasses RandomnessController | `phase3_synthesis/text_generator.py:308` |
| H6 | High | Randomness | Unseeded `random.randint()` | `phase5_mechanism/generators/constraint_geometry/table_variants.py:47-48` |
...
| H14 | High | Structure | 12+ pilot scripts duplicate `get_lines()` helper | `scripts/phase5_mechanism/run_5*_pilot.py` |
...
| H17 | High | Docs | SENSITIVITY_ANALYSIS.md plan never executed | `governance/SENSITIVITY_ANALYSIS.md` |
| H18 | High | Docs | governance/REPRODUCIBILITY.md missing Phase 4-5 | `governance/governance/REPRODUCIBILITY.md` |
