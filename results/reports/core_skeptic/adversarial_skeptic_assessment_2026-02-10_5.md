...
- `ALLOW_DIRTY_RELEASE=1 DIRTY_RELEASE_REASON='SK-PASS5 skeptic reassessment on intentional working tree' bash scripts/core_audit/pre_release_check.sh` -> **FAILED**
  - Release sensitivity preflight now runs and passes:
    ...
    - `scripts/core_audit/pre_release_check.sh:57`
...
- `bash scripts/ci_check.sh` -> **FAILED (policy stage before tests/coverage)**
  - CI policy chain passed through claim/control/closure/phase8_comparative/report/provenance checks.
...
- **PASS**: `python3 scripts/core_skeptic/check_claim_entitlement_coherence.py --mode release`
...
1. **Release preflight path is now active and passing across gate flows.**
   ...
   - Gate-health dependency snapshot now records preflight success:
     - `core_status/core_audit/release_gate_health_status.json:159`

2. **SK-H2/H1/H3/M2/M4 governance is now explicitly lane-based in canonical artifacts.**
   - H2 lane: `core_status/core_audit/release_gate_health_status.json:17`
   - H3 lane: `core_status/phase3_synthesis/CONTROL_COMPARABILITY_STATUS.json:97`
   - H1 lane: `results/phase5_mechanism/anchor_coupling_confirmatory.json:16`
   - M2 lane: `results/phase7_human/phase_7c_uncertainty.json:309`
   - M4 lane: `core_status/core_audit/provenance_health_status.json:25`
...
1. **Release sensitivity artifact/report pair is still missing.**
   ...
   - Release checker behavior remains fail-closed:
     ...
     - `scripts/core_audit/check_sensitivity_artifact_contract.py:280`
...
- Gate health records sensitivity failure family as active blockers:
  ...
  - `core_status/core_audit/release_gate_health_status.json:65`
...
- Canonical control comparability remains blocked at full-data scope:
  ...
  - `core_status/phase3_synthesis/CONTROL_COMPARABILITY_STATUS.json:18`
  - `core_status/phase3_synthesis/CONTROL_COMPARABILITY_STATUS.json:19`
- H3.4 lane and terminal feasibility are explicit:
  - `core_status/phase3_synthesis/CONTROL_COMPARABILITY_STATUS.json:91`
  - `core_status/phase3_synthesis/CONTROL_COMPARABILITY_STATUS.json:92`
  - `core_status/phase3_synthesis/CONTROL_COMPARABILITY_STATUS.json:97`
  - `core_status/phase3_synthesis/CONTROL_COMPARABILITY_STATUS.json:111`
- Data-availability artifact remains policy-consistent but blocked:
  ...
  - `core_status/phase3_synthesis/CONTROL_COMPARABILITY_DATA_AVAILABILITY.json:248`
  - `core_status/phase3_synthesis/CONTROL_COMPARABILITY_DATA_AVAILABILITY.json:264`
  - `core_status/phase3_synthesis/CONTROL_COMPARABILITY_DATA_AVAILABILITY.json:270`
  - `core_status/phase3_synthesis/CONTROL_COMPARABILITY_DATA_AVAILABILITY.json:287`
...
- Canonical status remains conclusive:
  ...
  - `results/phase5_mechanism/anchor_coupling_confirmatory.json:148`
- H1.4 lane is explicitly qualified with residual reason:
  - `results/phase5_mechanism/anchor_coupling_confirmatory.json:16`
  - `results/phase5_mechanism/anchor_coupling_confirmatory.json:17`
- Robustness remains mixed across registered lanes:
  - `results/phase5_mechanism/anchor_coupling_confirmatory.json:162`
  - `results/phase5_mechanism/anchor_coupling_confirmatory.json:184`
  - `results/phase5_mechanism/anchor_coupling_confirmatory.json:192`
...
- M2.4 lane and reopen triggers are now explicit:
  - `results/phase7_human/phase_7c_uncertainty.json:309`
  - `results/phase7_human/phase_7c_uncertainty.json:310`
- Top-2 fragility remains active:
  - `results/phase7_human/phase_7c_uncertainty.json:288`
  - `results/phase7_human/phase_7c_uncertainty.json:294`
  - `results/phase7_human/phase_7c_uncertainty.json:323`
