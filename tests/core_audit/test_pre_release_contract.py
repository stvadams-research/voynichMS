...
def test_pre_release_check_script_enforces_sensitivity_release_readiness() -> None:
    ...
    policy_path = Path("configs/core_audit/sensitivity_artifact_contract.json")
    assert policy_path.exists()
    assert "artifact_path" in policy_path.read_text(encoding="utf-8")
    provenance_policy = Path("configs/core_audit/provenance_runner_contract.json")
    assert provenance_policy.exists()
    assert "delegated_provenance" in provenance_policy.read_text(encoding="utf-8")
    multimodal_policy = Path("configs/core_skeptic/sk_h1_multimodal_status_policy.json")
...
def test_cleanup_status_artifacts_supports_dry_run_summary() -> None:
    script = Path("scripts/core_audit/cleanup_status_artifacts.sh").read_text(encoding="utf-8")
