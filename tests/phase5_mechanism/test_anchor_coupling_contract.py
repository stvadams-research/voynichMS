...
def test_anchor_coupling_script_emits_required_contract_fields() -> None:
    ...
    module = Path("src/phase5_mechanism/anchor_coupling.py").read_text(encoding="utf-8")
    ...
    assert "\"phase4_inference\"" in script
...
def test_coverage_audit_script_writes_machine_readable_output() -> None:
    script = Path("scripts/phase5_mechanism/audit_anchor_coverage.py").read_text(
        encoding="utf-8"
    )
    assert "core_status/phase5_mechanism/anchor_coverage_audit.json" in script
