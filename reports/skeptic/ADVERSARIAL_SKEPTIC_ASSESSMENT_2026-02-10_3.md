...
- `ALLOW_DIRTY_RELEASE=1 DIRTY_RELEASE_REASON='SK-PASS3 skeptic reassessment on intentional working tree' bash scripts/audit/pre_release_check.sh` -> **FAILED**
  ...
  - Enforced by:
    - `scripts/audit/check_sensitivity_artifact_contract.py:187`
    - `scripts/audit/check_sensitivity_artifact_contract.py:190`
    - `scripts/audit/check_sensitivity_artifact_contract.py:194`
    - `scripts/audit/pre_release_check.sh:48`
    - `scripts/audit/pre_release_check.sh:49`
    - `scripts/audit/pre_release_check.sh:54`
...
- `status/audit/release_gate_health_status.json:10` -> `GATE_HEALTH_DEGRADED`
- `status/audit/release_gate_health_status.json:11` -> `GATE_CONTRACT_BLOCKED`
- `status/audit/release_gate_health_status.json:13` -> `allowed_claim_class=QUALIFIED`
- `status/audit/release_gate_health_status.json:14` -> `allowed_closure_class=CONDITIONAL_CLOSURE_QUALIFIED`
...
- `status/audit/sensitivity_sweep.json:21` -> `quality_gate_passed=false`
- `status/audit/sensitivity_sweep.json:22` -> `robustness_conclusive=false`
- `status/audit/sensitivity_sweep.json:42` -> `robustness_decision="INCONCLUSIVE"`
- `status/audit/sensitivity_sweep.json:73` -> `release_readiness_failures` non-empty
- `status/audit/sensitivity_sweep.json:55` -> `dataset_pages=18`
- `status/audit/sensitivity_sweep.json:56` -> `dataset_tokens=216`
- `status/audit/sensitivity_sweep.json:28` -> `total_warning_count=270`
- `status/audit/sensitivity_sweep.json:29` -> `warning_density_per_scenario=54.0`
...
- `status/synthesis/CONTROL_COMPARABILITY_STATUS.json:33` -> `leakage_detected=false`
- `status/synthesis/CONTROL_COMPARABILITY_STATUS.json:41` -> `missing_count=4`
- `status/synthesis/CONTROL_COMPARABILITY_DATA_AVAILABILITY.json:12` -> `DATA_AVAILABILITY_BLOCKED`
...
- `results/mechanism/anchor_coupling_confirmatory.json:14` -> `status_reason="inferential_ambiguity"`
- `results/mechanism/anchor_coupling_confirmatory.json:15` -> no conclusive coupling claim allowed
- Adequacy is met:
  - `results/mechanism/anchor_coupling_confirmatory.json:86` -> `pass=true`
  - `results/mechanism/anchor_coupling_confirmatory.json:90`
  - `results/mechanism/anchor_coupling_confirmatory.json:91`
...
- `results/human/phase_7c_uncertainty.json:37` -> provisional-only allowed claim
- `results/human/phase_7c_uncertainty.json:40` -> `nearest_neighbor_stability=0.4565`
- `results/human/phase_7c_uncertainty.json:41` -> `jackknife_nearest_neighbor_stability=0.8333`
- `results/human/phase_7c_uncertainty.json:42` -> `rank_stability=0.4565`
- `results/human/phase_7c_uncertainty.json:50` -> `nearest_neighbor_probability_margin=0.067`
...
- `results/reports/PHASE_4_RESULTS.md:4` -> Phase 4 complete status
- `results/reports/PHASE_4_RESULTS.md:219` -> explicit statement that no contradictory pending-status blocks remain
- `results/reports/PHASE_4_STATUS_INDEX.json:7` -> coherence policy index present
...
- `status/audit/provenance_health_status.json:4` -> `HISTORICAL_ORPHANED_ROWS_PRESENT`
- `status/audit/provenance_health_status.json:19` -> `orphaned_rows=63`
- `status/audit/provenance_health_status.json:43` -> `contract_health_status=GATE_HEALTH_DEGRADED`
- `status/audit/provenance_health_status.json:45` -> `contract_reason_codes=[PROVENANCE_CONTRACT_BLOCKED]`
- `status/audit/provenance_health_status.json:48` -> `contract_coupling_pass=true`

Register sync state:

- `status/audit/provenance_register_sync_status.json:4` -> `status=IN_SYNC`
- `status/audit/provenance_register_sync_status.json:5` -> `drift_detected=false`
- `status/audit/provenance_register_sync_status.json:13` and `status/audit/provenance_register_sync_status.json:17` -> DB and artifact counts aligned
- `status/audit/provenance_register_sync_status.json:26` -> `contract_coupling_state=COUPLED_DEGRADED`
...
2. **Provenance register drift issue is not reproduced.**  
   Evidence: `status/audit/provenance_register_sync_status.json:4` (`IN_SYNC`) and `:5` (`drift_detected=false`).

3. **Claim-scope and closure language remain operationally qualified under degraded gate health.**  
   Evidence: `README.md:55`, `results/reports/PHASE_4_5_CLOSURE_STATEMENT.md:68`, `reports/comparative/PHASE_B_SYNTHESIS.md:43`.
