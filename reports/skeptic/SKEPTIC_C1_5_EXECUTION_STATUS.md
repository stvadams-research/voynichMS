...
- Updated `configs/audit/sensitivity_artifact_contract.json`:
...
- Updated `docs/SENSITIVITY_ANALYSIS.md`:
...
1. `BLOCKER-C1-REL-ARTIFACT` (OPEN)
   ...
   - Latest run-state evidence:
     - `status/audit/sensitivity_release_run_status.json` -> `status=FAILED`, `reason_codes=['RELEASE_RUN_FAILED']`
