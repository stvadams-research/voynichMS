...
1. `status/audit/release_gate_health_status.json`:
   - `status`
   - `allowed_claim_class`
   - `allowed_closure_class`
   - `h2_4_closure_lane`
   - freshness timestamp (`generated_utc`)
2. `configs/skeptic/sk_h2_claim_language_policy.json` (H2 lane mapping)
3. `configs/skeptic/sk_m1_closure_policy.json` (closure lane mapping)
4. Checker parity:
