...
- `python3 scripts/synthesis/run_test_a.py --seed 4242 --output /tmp/audit6_test_a_1.json` (run twice; canonical payload compare)
...
| INV-2 | Medium | Playbook-required `AUDIT_LOG.md` artifact is still missing. | Expectation: `planning/audit/CODE_AUDIT_AND_CLEANUP_PLAYBOOK.md:43`; not found via `rg --files -g 'AUDIT_LOG.md'` |
| INV-3 | Low | Repository keeps accumulating transient verification artifacts under `status/by_run/`, increasing local audit noise even though `status/` is ignored. | `status/by_run/*`; `.gitignore:39` |
...
|---|---|---|---|
| RI-1 | **Critical** | `run_indistinguishability_test.py` still uses commented-out computed logic and hardcoded/estimated values (`real_z`, `syn_z`, `real_rad`, `syn_rad`) that feed the pass/fail conclusion. | Commented-out metric runs: `scripts/synthesis/run_indistinguishability_test.py:97`; hardcoded/estimated values: `scripts/synthesis/run_indistinguishability_test.py:102`; conclusion logic: `scripts/synthesis/run_indistinguishability_test.py:129`; script is in reproducibility path: `docs/REPRODUCIBILITY.md:44` |
| RI-2 | **High** | Sensitivity artifacts remain legacy-reconciled and not evidence of a fresh canonical sweep (`dataset_id=unknown_legacy`, `pages=0`, `tokens=0`). | `reports/audit/SENSITIVITY_RESULTS.md:4`; `reports/audit/SENSITIVITY_RESULTS.md:5`; `status/audit/sensitivity_sweep.json:31`; `status/audit/sensitivity_sweep.json:33` |
| RI-3 | **High** | Sensitivity artifact provenance shows non-canonical execution command (`sensitivity_sweep_legacy_reconcile`) rather than `run_sensitivity_sweep`, so current published sensitivity evidence is not directly runner-derived. | `status/audit/sensitivity_sweep.json:10`; canonical runner config command is `scripts/analysis/run_sensitivity_sweep.py:427`; unresolved rerun noted in `reports/audit/FIX_EXECUTION_STATUS_5.md:161` |
| RI-4 | **High** | Reproducibility docs mark sensitivity status as executed with quality gates, but current published sensitivity outputs still reflect legacy/zero-data profile, causing documentation-evidence mismatch. | Claimed executed: `docs/SENSITIVITY_ANALYSIS.md:5`; legacy/zero-data output: `reports/audit/SENSITIVITY_RESULTS.md:4`; `reports/audit/SENSITIVITY_RESULTS.md:6` |

### 1.2 Medium/Low findings

| ID | Severity | Finding | Location |
|---|---|---|---|
| RI-5 | Medium | Verification workflow mutates primary `data/voynich.db` (not an isolated test DB), allowing verification runs to alter baseline state. | `scripts/verify_reproduction.sh:16`; `scripts/verify_reproduction.sh:32`; `scripts/synthesis/run_test_a.py:38`; `scripts/synthesis/run_test_a.py:63` |
| RI-6 | Medium | Strict no-fallback mode (`REQUIRE_COMPUTED=1`) remains optional in reproducibility guidance while fallback defaults are present in core synthesis profile extraction. | Optional strict mode: `docs/REPRODUCIBILITY.md:93`; fallback defaults and simulated page data: `src/synthesis/profile_extractor.py:71`; `src/synthesis/profile_extractor.py:102` |
| RI-7 | Low | Residual debug/commented diagnostic code remains in execution path script. | `scripts/mechanism/run_5i_anchor_coupling.py:58` |
...
| ID | Severity | Finding | Evidence |
|---|---|---|---|
| MC-1 | **High** | Provenance snapshots frequently persist `status: running` because metadata is captured before run completion. | Status sourced from active run: `src/foundation/core/provenance.py:27`; save-before-complete pattern: `scripts/synthesis/run_test_a.py:121` then `scripts/synthesis/run_test_a.py:123`; artifact example `running`: `status/by_run/verify_1.1291aeff-2f29-02e2-551d-1aa8b0ea6912.json:6`; corresponding run manifest `success`: `runs/1291aeff-2f29-02e2-551d-1aa8b0ea6912/run.json:3` |
| MC-2 | **High** | Run records in metadata DB are systematically stale (`status=running`, `timestamp_end=NULL`) for completed runs, weakening run-history trustworthiness. | `MetadataStore.save_run` records current run status: `src/foundation/storage/metadata.py:489`; run completion occurs later in context manager: `src/foundation/runs/manager.py:178`; DB query evidence shows widespread `running` statuses |
| MC-3 | **High** | Coverage gate passes at 40% despite important modules remaining effectively untested (including 0% coverage in key components). | Gate config `scripts/ci_check.sh:20`; full-suite output still includes 0% files (e.g., `src/foundation/cli/main.py`, `src/analysis/admissibility/manager.py`, `src/foundation/core/queries.py`) |
| MC-4 | Medium | Sensitivity guardrail tests validate helper logic but do not provide end-to-end proof that published sensitivity artifacts are generated by canonical runner execution. | `tests/analysis/test_sensitivity_sweep_guardrails.py:14`; `tests/analysis/test_sensitivity_sweep_guardrails.py:43` |
...
| ID | Severity | Finding | Location |
|---|---|---|---|
| ST-1 | Medium | Output contracts are still mixed in practice: provenance-managed JSON exists, but audit-significant sensitivity artifacts currently reflect legacy reconciliation path and transient `status/` location. | `status/audit/sensitivity_sweep.json:10`; `docs/PROVENANCE.md:67`; `reports/audit/SENSITIVITY_RESULTS.md:4` |
| ST-2 | Low | Human-facing message drift in `run_indistinguishability_test.py` (declares “18 pages” while generating count=2) indicates stale script narration. | Generation count: `scripts/synthesis/run_indistinguishability_test.py:60`; message: `scripts/synthesis/run_indistinguishability_test.py:86` |
...
| ID | Severity | Finding | Location |
|---|---|---|---|
| DOC-1 | **High** | Documentation and current sensitivity evidence are misaligned: docs present executed quality-gated process while extant outputs remain legacy/zero-data reconciled. | `docs/SENSITIVITY_ANALYSIS.md:5`; `reports/audit/SENSITIVITY_RESULTS.md:4`; `reports/audit/FIX_EXECUTION_STATUS_5.md:162` |
| DOC-2 | Medium | Playbook output contract is only partially met; `METHODS_REFERENCE`/`CONFIG_REFERENCE`/`REPRODUCIBILITY` exist, but required `AUDIT_LOG.md` is absent. | Expected output list: `planning/audit/CODE_AUDIT_AND_CLEANUP_PLAYBOOK.md:43`; existing files: `docs/METHODS_REFERENCE.md`, `docs/CONFIG_REFERENCE.md`, `docs/REPRODUCIBILITY.md` |
