...
| `status/by_run` files | 20 | `find status/by_run -type f` |
...
|---|---|---|---|
| INV-1 | Medium | Worktree remains non-clean (`89` modified/untracked paths), and release gate fails without explicit override. | `scripts/audit/pre_release_check.sh:40`, `scripts/audit/pre_release_check.sh:45`; `git status --short` |
...
|---|---|---|---|
| RI-10 | **High** | Sensitivity evidence can be marked `release_evidence_ready=true` while robustness is explicitly `INCONCLUSIVE` and quality gates fail; release checks currently treat this as pass. | Robustness fields: `status/audit/sensitivity_sweep.json:23`, `status/audit/sensitivity_sweep.json:24`; release-ready flag: `status/audit/sensitivity_sweep.json:37`; report caveats: `reports/audit/SENSITIVITY_RESULTS.md:11`, `reports/audit/SENSITIVITY_RESULTS.md:16`, `reports/audit/SENSITIVITY_RESULTS.md:17`, `reports/audit/SENSITIVITY_RESULTS.md:22`, `reports/audit/SENSITIVITY_RESULTS.md:23`; gate logic checks only mode/release-ready/scenario-count: `scripts/audit/pre_release_check.sh:30`, `scripts/audit/pre_release_check.sh:35`; same contract in verifier: `scripts/verify_reproduction.sh:137`, `scripts/verify_reproduction.sh:149`; observed `pre_release_check` pass under `ALLOW_DIRTY_RELEASE=1` despite `INCONCLUSIVE` robustness |
| RI-11 | **High** | Strict release-evidentiary indistinguishability path is blocked by missing real-data prerequisites (`10/18` pages missing). | Strict preflight guard: `scripts/synthesis/run_indistinguishability_test.py:84`, `scripts/synthesis/run_indistinguishability_test.py:111`; strict command in reproducibility docs: `docs/REPRODUCIBILITY.md:44`, `docs/REPRODUCIBILITY.md:94`; observed runtime failure message with `REQUIRE_COMPUTED=1` |

### 1.2 Medium findings

| ID | Severity | Finding | Location |
|---|---|---|---|
| RI-12 | Medium | Non-strict indistinguishability remains default (`REQUIRE_COMPUTED` opt-in), so fallback/simulated paths can still run unless caller enforces strict mode. | `scripts/synthesis/run_indistinguishability_test.py:125`, `scripts/synthesis/run_indistinguishability_test.py:114` |
...
| MC-2R | Medium | Historical run provenance remains uncertain for old executions: `63` runs are now explicitly `orphaned` (no open `running` rows), and none have `runs/<id>/run.json`. | SQLite query on `data/voynich.db`: `success=89`, `orphaned=63`, `running_end_null=0`, `orphaned_missing_run_json=63`; policy note: `docs/PROVENANCE.md:88` |
| ST-1 | Medium | Legacy `status/by_run/verify_*.json` payloads still contain `provenance.status: running` (`20/20` files), which conflicts with current immutable-artifact policy language. | Example file: `status/by_run/verify_1.1291aeff-2f29-02e2-551d-1aa8b0ea6912.json:6`; policy: `docs/PROVENANCE.md:45`, `docs/PROVENANCE.md:49` |
| DOC-4 | Medium | Playbook expected outputs list root-level `METHODS_REFERENCE.md`/`CONFIG_REFERENCE.md`/`REPRODUCIBILITY.md`, while project currently uses `docs/` paths; contract is workable but naming/path mismatch remains for strict checklist interpretation. | Playbook contract: `planning/audit/CODE_AUDIT_AND_CLEANUP_PLAYBOOK.md` (Expected Outputs section); current files: `docs/METHODS_REFERENCE.md`, `docs/CONFIG_REFERENCE.md`, `docs/REPRODUCIBILITY.md` |
...
| RI-9 partial (bounded sweep) | **Improved** | Sensitivity artifact now executes full release scenario count `17/17` with canonical command. | `status/audit/sensitivity_sweep.json:9`, `status/audit/sensitivity_sweep.json:35`, `status/audit/sensitivity_sweep.json:36`; `reports/audit/SENSITIVITY_RESULTS.md:8` |
...
| ID | Severity | Finding | Location |
|---|---|---|---|
| ST-1 | Medium | Historical verify artifacts still encode legacy `provenance.status: running` state. | `status/by_run/verify_1.1291aeff-2f29-02e2-551d-1aa8b0ea6912.json:6` |
...
| ID | Severity | Finding | Location |
|---|---|---|---|
| DOC-1 | Improved | Reproducibility documentation now describes verifier sentinel contract and strict-mode command. | `docs/REPRODUCIBILITY.md:124`, `docs/REPRODUCIBILITY.md:126`, `docs/REPRODUCIBILITY.md:131` |
| DOC-4 | Medium | Expected-output filenames in playbook and actual docs pathing differ (`docs/` vs root-level names), which can cause checklist ambiguity. | `planning/audit/CODE_AUDIT_AND_CLEANUP_PLAYBOOK.md`; `docs/METHODS_REFERENCE.md`; `docs/CONFIG_REFERENCE.md`; `docs/REPRODUCIBILITY.md` |
...
|---|---|
| Where are assumptions stated? | Better documented in `docs/PROVENANCE.md`, `docs/SENSITIVITY_ANALYSIS.md`, and sensitivity report caveats. |
...
- Decide and document policy for historical orphaned runs and legacy verify artifacts (`status/by_run`) prior to publication freeze.
