...
4. **No CONFIG_REFERENCE.md, METHODS_REFERENCE.md, or REPRODUCIBILITY.md** (High) - External reader cannot modify, understand, or reproduce
...
- **DEBUG print in production:** `src/foundation/anchors/engine.py:77` - `print(f"DEBUG: ...")` left in anchor computation
- **TODO in baseline assessment:** `scripts/synthesis/run_baseline_assessment.py` - "Info Density and Locality require control datasets"
- **Hypothesis manager:** `src/foundation/hypotheses/manager.py` - "Instantiate temporarily to get ID?"
...
**Perturbation Battery (src/analysis/models/disconfirmation.py:40-59)**
...
**Hypothesis Falsification Thresholds (src/foundation/hypotheses/destructive.py)**
...
**Evaluation Weights (src/analysis/models/evaluation.py:113-118)**
...
**Capacity Bounding Constants (src/analysis/anomaly/capacity_bounding.py:38-42)**
...
**Stability Baselines (src/analysis/anomaly/stability_analysis.py:38-46)**
...
**Synthesis Constraints (src/synthesis/interface.py:158-167)**
...
#### Critical (2)
- **Bare `except:` clauses** in `src/human/quire_analysis.py:29`, `src/human/scribe_coupling.py:32`, `src/inference/network_features/analyzer.py:51` - catch all exceptions including SystemExit, silently return default values (0, "Unknown", 0.0)

#### High (10)
- **src/foundation/metrics/library.py:169,208** - `except Exception: continue` silently drops corrupted embeddings from ClusterTightness calculation
- **src/synthesis/refinement/feature_discovery.py:92-697** - 12+ functions return `random.uniform(...)` for scrambled pages or hardcoded defaults when data is None, with no indication to caller
- **src/foundation/metrics/library.py:64-65** - Silent fallback from TranscriptionTokenRecord to WordAlignmentRecord without warning
- **src/analysis/models/perturbation.py:119-135** - `.get()` with defaults (0, 1) creates phantom bounding boxes when coordinates missing
- **src/synthesis/profile_extractor.py:82-83** - `store is None` silently returns simulated profile
- **src/synthesis/refinement/feature_discovery.py:101** - Returns `float("nan")` which propagates silently through calculations (NaN comparisons always False)
- **src/foundation/hypotheses/destructive.py:440** - `std_control` defaults to 0.1 if variance is 0 (arbitrary smoothing factor)
...
A `RandomnessController` exists at `src/foundation/core/randomness.py` with three modes (FORBIDDEN, SEEDED, UNRESTRICTED) and enforcement decorators. However, adoption is inconsistent - most modules don't use it.

#### Critical (Unseeded, impacts core results)

| File | Lines | Issue |
|---|---|---|
| `src/synthesis/refinement/feature_discovery.py` | 93-697 | 12+ `random.uniform()` calls for feature computation - UNSEEDED |
| `src/synthesis/generators/grammar_based.py` | 48 | Core text generation uses unseeded `random.choices()` |
| `src/synthesis/indistinguishability.py` | 132-140 | Scrambled control generation is non-deterministic |

#### High (Unseeded mechanism simulators)

| File | Unseeded Calls | Purpose |
|---|---|---|
| `src/mechanism/workflow/simulators.py` | 5 | Gaussian sampling, reservoir sampling, drift |
| `src/mechanism/large_object/simulators.py` | 2 | Grid start position |
| `src/mechanism/topology_collapse/simulators.py` | 6 | Grid, layered table, DAG, lattice start positions |
| `src/mechanism/entry_selection/simulators.py` | 5 | Entry point and coupling mechanisms |
| `src/mechanism/generators/pool_generator.py` | 3 | Pool reuse and drift |
| `src/mechanism/dependency_scope/simulators.py` | 3 | Dependency scope testing |
| `src/mechanism/parsimony/simulators.py` | 2 | Parsimony evaluation |
...
- **Voynichese token seeds:** `src/synthesis/profile_extractor.py:626-637` contains hardcoded Voynich-like tokens ("daiin", "chedy", "qokedy") used in fallback path when transcription database unavailable. Could seed biased synthesis.
...
- **Scrambled control generation:** Implementation exists in `src/foundation/controls/scramblers.py` using `random.Random(seed)` (properly isolated), but mechanism could be better documented

#### Mapping Reference Point
- `src/comparative/mapping.py:15` hardcodes `target='Voynich'` as reference point for distance calculations - this is by design (comparisons need a reference)

### 1.7 Output Provenance Metadata

#### Run-Level Provenance: COMPREHENSIVE
`src/foundation/runs/context.py` captures:
...
**Test infrastructure exists** at `/tests/` with proper directory structure matching `/src/`. An acceptance test exists at `scripts/foundation/acceptance_test.py`.
...
- Data assets (`data/`)
- Documentation (`docs/`, `planning/`, `results/`)
- Tests (`tests/`)

No Jupyter notebooks found. All logic is importable from `src/` modules.

**Issues:**
- **Multiple pilot scripts:** 15 scripts in `scripts/mechanism/` with naming like `run_5b_pilot.py` through `run_5k_pilot.py`. These are sequential experiments, not duplicates, but naming convention is unclear to outsiders.
...
- `foundation/cli/main.py:218` - `query_token(token: str)` for transcription units
- `foundation/cli/main.py:236` - `query_word(word_id: str)` for image units
...
|---|---|---|---|
| `foundation/anchors/engine.py` | 77 | `print(f"DEBUG: ...")` | HIGH - Debug output in production |
| `foundation/qc/reporting.py` | 8,16 | `print(f"Generating...")` | LOW - Stubs |
| `foundation/qc/anomalies.py` | 25 | `print(f"WARNING: ...")` | MEDIUM |
| `foundation/core/dataset.py` | 66 | `print(f"Skipping...")` | MEDIUM |
| `foundation/analysis/sensitivity.py` | 34 | `print(f"Error...")` | MEDIUM |
...
**No METHODS_REFERENCE.md exists.**

`docs/METRICS.md` provides high-level metric definitions (4 metrics). Code has inline docstrings. But:
...
**No REPRODUCIBILITY.md exists.**

`docs/RUNBOOK.md` is skeletal (41 lines), covers Phases 2-3 only. No end-to-end reproduction script. No expected output specifications. No failure mode guide. No minimal reproducible example. No verification checklist.
...
A: `planning/foundation/PRINCIPLES_AND_NONGOALS.md` states core assumptions. However, 150+ hardcoded thresholds are not documented as assumptions. A skeptical reader would question every threshold in Phase 1.2 above.
...
A: Unknown. No sensitivity sweep has been performed or documented. The `foundation/analysis/sensitivity.py` module exists but has a print-based error handler, suggesting it may not be production-ready.
...
1. No REPRODUCIBILITY.md exists
...
| C2 | 1.4 | Grammar-based generator uses unseeded random.choices() | synthesis/generators/grammar_based.py:48 |
| C3 | 1.4 | Feature discovery uses 12+ unseeded random.uniform() calls | synthesis/refinement/feature_discovery.py |
| C4 | 1.4 | Indistinguishability test controls are non-deterministic | synthesis/indistinguishability.py:132-140 |
...
| C12 | 1.4 | 8+ mechanism simulators completely unseeded | mechanism/*/simulators.py |
...
| H13 | 4.2 | No METHODS_REFERENCE.md | N/A |
| H14 | 4.4 | No REPRODUCIBILITY.md | N/A |
...
| H22 | 1.4 | Pool generator, workflow simulators unseeded | mechanism/generators/, mechanism/workflow/ |
...
| H31 | 4.4 | RUNBOOK.md is skeletal (41 lines, Phases 2-3 only) | docs/RUNBOOK.md |
...
1. `src/synthesis/generators/grammar_based.py` - Add seed propagation
2. `src/synthesis/refinement/feature_discovery.py` - Seed all random calls
3. `src/synthesis/indistinguishability.py` - Seed scrambled control generation
4. `src/analysis/stress_tests/mapping_stability.py:113` - Fix boolean truthiness bug
5. All mechanism simulators (8+ files) - Add seed parameters

### Before External Review
6. Create `docs/CONFIG_REFERENCE.md`
7. Create `docs/METHODS_REFERENCE.md`
8. Create `docs/REPRODUCIBILITY.md`
9. Remove DEBUG print from `src/foundation/anchors/engine.py:77`
