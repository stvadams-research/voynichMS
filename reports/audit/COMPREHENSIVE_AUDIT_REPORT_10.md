...
- Determinism replay for Test A (two `scripts/synthesis/run_test_a.py` runs with same seed; canonical `results` compare)
- `python3 -m pytest --cov=src --cov-report=term-missing:skip-covered -q tests`
- `bash scripts/audit/pre_release_check.sh`
- `ALLOW_DIRTY_RELEASE=1 DIRTY_RELEASE_REASON='Audit10 assessment' bash scripts/audit/pre_release_check.sh`
...
|---|---|---|---|
| INV-1 | Medium | Worktree remains non-clean (`109` modified/untracked paths). | `scripts/audit/pre_release_check.sh:136`, `scripts/audit/pre_release_check.sh:149` |
| INV-2 | Resolved | Playbook-required audit log exists. | `scripts/audit/pre_release_check.sh:6`, `scripts/audit/pre_release_check.sh:11` |
| INV-3 | Resolved | Legacy `status/by_run` verification artifacts are absent in current workspace state. | `scripts/audit/pre_release_check.sh:113` |
...
|---|---|---|---|
| RI-13 | **High** | Release-evidence sensitivity artifact remains non-conclusive and non-ready, blocking verifier/CI/release gates. | `status/audit/sensitivity_sweep.json:21`, `status/audit/sensitivity_sweep.json:22`, `status/audit/sensitivity_sweep.json:26`, `status/audit/sensitivity_sweep.json:39`; gate enforcement: `scripts/audit/pre_release_check.sh:26`, `scripts/audit/pre_release_check.sh:42`, `scripts/audit/pre_release_check.sh:47`, `scripts/audit/pre_release_check.sh:52`; verifier enforcement: `scripts/verify_reproduction.sh:129`, `scripts/verify_reproduction.sh:153`, `scripts/verify_reproduction.sh:157`, `scripts/verify_reproduction.sh:161` |

### 1.2 Medium findings

| ID | Severity | Finding | Evidence |
|---|---|---|---|
| RI-11 (criteria retained) | **Medium** | Strict indistinguishability remains blocked by unavailable/lost source pages (`f91r`, `f91v`, `f92r`, `f92v`) and is treated as a scoped data-availability constraint, not a pure implementation defect. | `status/synthesis/by_run/TURING_TEST_RESULTS.82412819-f2cf-88b3-b74b-01ec27e4bb05.json:12`, `status/synthesis/by_run/TURING_TEST_RESULTS.82412819-f2cf-88b3-b74b-01ec27e4bb05.json:14`, `status/synthesis/by_run/TURING_TEST_RESULTS.82412819-f2cf-88b3-b74b-01ec27e4bb05.json:15`, `status/synthesis/by_run/TURING_TEST_RESULTS.82412819-f2cf-88b3-b74b-01ec27e4bb05.json:488`, `status/synthesis/by_run/TURING_TEST_RESULTS.82412819-f2cf-88b3-b74b-01ec27e4bb05.json:494`; policy docs: `docs/REPRODUCIBILITY.md:135`, `docs/REPRODUCIBILITY.md:136`, `docs/REPRODUCIBILITY.md:137`, `docs/PROVENANCE.md:84`, `docs/PROVENANCE.md:87` |
| RI-12 | Medium | Non-strict fallback path is still default unless strict mode is explicitly enabled (`REQUIRE_COMPUTED=1` or `--strict-computed`). | strict mode derivation: `scripts/synthesis/run_indistinguishability_test.py:420`; CLI flag: `scripts/synthesis/run_indistinguishability_test.py:406`, `scripts/synthesis/run_indistinguishability_test.py:413`; non-strict artifact: `status/synthesis/by_run/TURING_TEST_RESULTS.f4bd9cb2-ea8b-fea0-b2af-34ca44178fdd.json:13` |
| MC-3 | Medium | Aggregate coverage exceeds threshold (`56.08%`), but one synthesis module remains below 20% (`feature_discovery.py` at `12.85%`), leaving low test confidence in that path. | `status/ci_coverage.json:1` (totals and per-file summaries), full run evidence from `bash scripts/ci_check.sh` output |
| MC-2R | Medium | Historical run provenance uncertainty persists: SQLite `runs` table still contains `63` `orphaned` rows and filesystem manifest check still shows `63` missing `runs/<id>/run.json`. | SQLite queries on `data/voynich.db` (`SELECT status, COUNT(*) ...` and manifest presence check); policy framing: `docs/PROVENANCE.md:96`, `docs/PROVENANCE.md:101`, `docs/PROVENANCE.md:104` |
| INV-1 | Medium | Release-cut baseline remains non-clean (`109` working-tree changes). | `git status --short | wc -l`; gate path: `scripts/audit/pre_release_check.sh:136`, `scripts/audit/pre_release_check.sh:149` |

### 1.3 Resolved/Improved since Audit 9

| Prior ID | Current state | Evidence |
|---|---|---|
| RI-14 (split-folio overcount) | **Resolved** | Preflight now canonicalizes split folios and reports missing `4/18` pages instead of inflated counts. | normalization logic: `scripts/synthesis/run_indistinguishability_test.py:129`, `scripts/synthesis/run_indistinguishability_test.py:133`, `scripts/synthesis/run_indistinguishability_test.py:144`, `scripts/synthesis/run_indistinguishability_test.py:147`; artifact fields: `status/synthesis/by_run/TURING_TEST_RESULTS.f4bd9cb2-ea8b-fea0-b2af-34ca44178fdd.json:260`, `status/synthesis/by_run/TURING_TEST_RESULTS.f4bd9cb2-ea8b-fea0-b2af-34ca44178fdd.json:486`, `status/synthesis/by_run/TURING_TEST_RESULTS.f4bd9cb2-ea8b-fea0-b2af-34ca44178fdd.json:492` |
| DOC-5 (RI-11 policy documentation gap) | **Resolved** | RI-11 policy treatment is now explicitly documented in reproducibility and provenance docs. | `docs/REPRODUCIBILITY.md:135`, `docs/REPRODUCIBILITY.md:136`, `docs/REPRODUCIBILITY.md:137`; `docs/PROVENANCE.md:84`, `docs/PROVENANCE.md:87`, `docs/PROVENANCE.md:88`, `docs/PROVENANCE.md:89` |
| MC-3 partial | **Improved** | Coverage increased from prior pass baseline to `56.08%`; under-20 modules reduced to one. | `status/ci_coverage.json:1`; pytest/CI outputs from this pass |
| RI-10 (gate false-pass semantics) | **Still resolved** | Release checks remain fail-closed when sensitivity is not release-ready/conclusive/quality-passing. | `scripts/audit/pre_release_check.sh:37`, `scripts/audit/pre_release_check.sh:42`, `scripts/audit/pre_release_check.sh:47`, `scripts/audit/pre_release_check.sh:52`; `scripts/verify_reproduction.sh:144`, `scripts/verify_reproduction.sh:153`, `scripts/verify_reproduction.sh:157`, `scripts/verify_reproduction.sh:161` |
...
| ID | Severity | Finding | Location |
|---|---|---|---|
| ST-3 | Resolved | Preflight now uses normalized page-ID handling for split folios, reducing naming/identity drift in strict checks. | `scripts/synthesis/run_indistinguishability_test.py:69`, `scripts/synthesis/run_indistinguishability_test.py:129`, `scripts/synthesis/run_indistinguishability_test.py:144` |
...
| ID | Severity | Finding | Location |
|---|---|---|---|
| DOC-1 | Improved | Strict mode commands and release-evidence constraints remain documented in reproducibility guide. | `docs/REPRODUCIBILITY.md:90`, `docs/REPRODUCIBILITY.md:94`, `docs/REPRODUCIBILITY.md:101`, `docs/REPRODUCIBILITY.md:135` |
| DOC-5 | Resolved | RI-11 data-availability interpretation is now explicitly encoded in release-facing docs. | `docs/REPRODUCIBILITY.md:136`, `docs/REPRODUCIBILITY.md:137`; `docs/PROVENANCE.md:87`, `docs/PROVENANCE.md:88`, `docs/PROVENANCE.md:89` |
