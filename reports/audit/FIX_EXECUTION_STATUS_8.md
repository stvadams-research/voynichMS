...
| WS-D Provenance/Run-State Reconciliation | COMPLETE | Added legacy artifact checks to release gates and improved cleanup/report tooling behavior. |
| WS-E Release Baseline Hygiene | COMPLETE | `ALLOW_DIRTY_RELEASE=1` now requires `DIRTY_RELEASE_REASON`. |
| WS-F Documentation Contract Alignment | COMPLETE | Added root-level `METHODS_REFERENCE.md`, `CONFIG_REFERENCE.md`, `REPRODUCIBILITY.md` aliases to canonical `docs/` paths. |
...
- `reports/audit/SENSITIVITY_RESULTS.md` (regenerated)
- `status/audit/sensitivity_sweep.json` (regenerated)
...
- `scripts/analysis/run_sensitivity_sweep.py:286`
- `scripts/analysis/run_sensitivity_sweep.py:383`
- `scripts/analysis/run_sensitivity_sweep.py:541`
- `scripts/audit/pre_release_check.sh:35`
...
- Added strict blocked-result emission to `status/synthesis/TURING_TEST_RESULTS.json` when strict preflight fails.
- Added CLI options:
  - `--preflight-only`
  - `--strict-computed`
...
- `scripts/synthesis/run_indistinguishability_test.py:85`
- `scripts/synthesis/run_indistinguishability_test.py:126`
- `scripts/synthesis/run_indistinguishability_test.py:142`
- `scripts/synthesis/run_indistinguishability_test.py:347`

### WS-C: Coverage hardening

Updated files (new tests):

- `tests/foundation/core/test_logging_core.py`
- `tests/foundation/core/test_models.py`
- `tests/foundation/qc/test_anomalies.py`
- `tests/foundation/qc/test_checks.py`
- `tests/foundation/storage/test_filesystem.py`
- `tests/foundation/storage/test_interface.py`
...
- `src/foundation/storage/filesystem.py:64`

### WS-D: Provenance and artifact lifecycle

Updated files:

- `scripts/audit/cleanup_status_artifacts.sh`
- `scripts/audit/pre_release_check.sh`
- `scripts/verify_reproduction.sh`
- `docs/PROVENANCE.md`
...
- Added release-gate checks that fail if `status/by_run/verify_*.json` include legacy `provenance.status` fields.

Key references:

- `scripts/audit/cleanup_status_artifacts.sh:10`
- `scripts/audit/cleanup_status_artifacts.sh:36`
...
- `scripts/audit/pre_release_check.sh:95`
- `docs/REPRODUCIBILITY.md:176`

### WS-F: Documentation contract alignment

Updated files:

- `METHODS_REFERENCE.md` (new root alias)
- `CONFIG_REFERENCE.md` (new root alias)
- `REPRODUCIBILITY.md` (new root alias)
...
```bash
python3 -m pytest -q tests/analysis/test_sensitivity_sweep_guardrails.py tests/analysis/test_sensitivity_sweep_end_to_end.py tests/audit/test_pre_release_contract.py tests/audit/test_verify_reproduction_contract.py tests/synthesis/test_run_indistinguishability_runner.py tests/foundation/core/test_logging_core.py tests/foundation/core/test_models.py tests/foundation/qc/test_anomalies.py tests/foundation/qc/test_checks.py tests/foundation/storage/test_filesystem.py tests/foundation/storage/test_interface.py
```
...
```bash
python3 scripts/analysis/run_sensitivity_sweep.py --dataset-id voynich_synthetic_grammar --mode release
```
...
python3 scripts/synthesis/run_indistinguishability_test.py --strict-computed --preflight-only
```
...
6. Coverage delta for prior 0%-coverage modules (`status/ci_coverage.json`):

- `src/foundation/core/logging.py`: `94.12%`
- `src/foundation/core/models.py`: `100.00%`
- `src/foundation/qc/anomalies.py`: `100.00%`
- `src/foundation/qc/checks.py`: `100.00%`
- `src/foundation/storage/filesystem.py`: `91.49%`
- `src/foundation/storage/interface.py`: `80.00%`
...
- Plan tracker statuses were updated in `planning/audit/AUDIT_8_EXECUTION_PLAN.md`.
- Audit execution notes were appended to `AUDIT_LOG.md` under Audit 8.
- This execution run intentionally focused on remediation; the next assessment pass should be produced separately as `reports/audit/COMPREHENSIVE_AUDIT_REPORT_9.md`.
