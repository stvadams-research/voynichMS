...
- **Used in:** `MontemurroAnalyzer` (`src/inference/info_clustering/analyzer.py`)
...
- **Used in:** `MorphologyAnalyzer` (`src/inference/morph_induction/analyzer.py`)
...
- **Used in:** `MappingStabilityTester` (`src/analysis/stress_tests/mapping_stability.py`)
...
- **Used in:** `DisconfirmationEngine` (`src/analysis/models/disconfirmation.py`)
- **Rationale:**
...
- **Used in:** `EquivalenceTest` (`src/synthesis/refinement/interface.py`)
...
## 5. Centralized Analysis Thresholds (`configs/analysis/thresholds.json`)
