...
- **Analysis:** `analysis/` (Sensitivity, stability) - CRITICAL
...
## 3. Entry Points
- CLI: `src/foundation/cli/main.py`
- Scripts: `scripts/` directory (various runner scripts)

## 4. Candidates for Cleanup/Deprecation
- `src/foundation/regions/dummy.py`: "Dummy" implementations might belong in tests or be renamed "Reference".
- `src/foundation/segmentation/dummy.py`: Same as above.
- `src/synthesis/refinement/`: Verify if these complex refinement modules are actually used in the current pipeline or if they were from an earlier iteration.
