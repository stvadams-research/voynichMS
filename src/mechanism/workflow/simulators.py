...
class LineScopedPoolSimulator:
    ...
    """
    def __init__(self, grammar_path: Path, mean_pool_size: float = 10.0):
    ...
    def generate_line(self, target_len: int) -> List[str]:
        ...
        pool_size = max(1, int(random.gauss(self.mean_pool_size, 2.0)))
        
        # 2. Populate pool
        pool = [self.generator.generate_word() for _ in range(pool_size)]
        
        # 3. Fill line
        return [random.choice(pool) for _ in range(target_len)]
...
class WeaklyCoupledPoolSimulator:
    ...
    """
    def __init__(self, grammar_path: Path, reservoir_size: int = 50, drift_rate: float = 0.1):
        self.generator = GrammarBasedGenerator(grammar_path)
    ...
    def generate_line(self, target_len: int) -> List[str]:
        ...
        pool = random.sample(self.reservoir, min(pool_size, len(self.reservoir)))
        
        line = [random.choice(pool) for _ in range(target_len)]
