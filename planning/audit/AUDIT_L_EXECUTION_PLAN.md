...
|---|---|---|
| **L1** | Hardcoded seeds in simulators | Replace `seed=42` with a configurable parameter or move to `foundation/config.py`. |
...
| **L4** | Style inconsistencies | Align whitespace and docstring formatting across the `mechanism/` submodules. |
...
| **L11** | Glossary update | Add "MetricResult" and "HypothesisResult" definitions to `docs/GLOSSARY.md`. |
| **L12** | Command help text | Refine `typer` help strings in `foundation/cli/main.py` for clarity. |
