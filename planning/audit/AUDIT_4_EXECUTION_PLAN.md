...
|---|---|---|---|
| A1 | Seed Test A grammar generator explicitly and thread seed through all random generation paths. | `scripts/synthesis/run_test_a.py`, `src/synthesis/generators/grammar_based.py` | Repeated Test A runs with same seed produce identical metric payloads. |
| A2 | Remove unstable use of Python `hash()` for RNG seed derivation; replace with stable deterministic hash (for example SHA256-derived int). | `src/synthesis/text_generator.py`, `src/synthesis/refinement/feature_discovery.py` | Seed-derived behavior is stable across interpreter restarts. |
...
| A5 | Add deterministic regression test for Test A output consistency. | `tests/audit/` | Automated test catches future nondeterminism regressions. |

### Verification Commands (to run during execution)

```bash
python scripts/synthesis/run_test_a.py
cp status/synthesis/TEST_A_RESULTS.json /tmp/test_a_1.json
python scripts/synthesis/run_test_a.py
cp status/synthesis/TEST_A_RESULTS.json /tmp/test_a_2.json
...
| Task ID | Action | Target Files | Exit Criteria |
|---|---|---|---|
| B1 | Fix missing `Tuple` import in config loader return annotation. | `src/foundation/configs/loader.py` | Module imports successfully in isolation. |
| B2 | Ensure `RunContext` default construction path is valid or explicitly prohibited with clear error contract. | `src/foundation/runs/context.py`, `src/foundation/core/ids.py`, `src/foundation/runs/manager.py` | `RunContext` usage is consistent and no hidden constructor trap remains. |
| B3 | Add targeted unit tests for loader import and run context lifecycle behavior. | `tests/foundation/` | Regressions caught automatically. |
...
python -m pytest -q tests/foundation
```
...
| Task ID | Action | Target Files | Exit Criteria |
|---|---|---|---|
| C1 | Redesign run identity model to avoid seed-only run ID collisions. Keep deterministic experiment linkage separately from unique run instance ID. | `src/foundation/runs/manager.py`, `src/foundation/core/ids.py`, `src/foundation/storage/metadata.py` | Repeated same-seed runs generate distinct run records without overwrite. |
| C2 | Update result-writing conventions so outputs are run-scoped or append-only, not static overwrite paths. | `scripts/*/run_*.py`, `src/foundation/core/provenance.py` | Running same script twice preserves both outputs. |
...
| C4 | Add compatibility strategy for existing consumers expecting old static filenames (index file or latest symlink/pointer). | `scripts/`, `docs/` | Backward compatibility maintained with explicit policy. |
...
| Task ID | Action | Target Files | Exit Criteria |
|---|---|---|---|
| D1 | Complete baseline synthesis assessment metrics currently marked `NOT COMPUTED` (or enforce explicit fail-fast if prerequisites missing). | `scripts/synthesis/run_baseline_assessment.py` | No silent placeholder outputs in baseline report path. |
| D2 | Make language-ID transform randomization seed-controlled and deterministic per run configuration. | `scripts/inference/run_lang_id.py` | Same seed yields same transform set and scores. |
| D3 | Fix token ingestion remainder loss by ingesting final partial page/chunk. | `scripts/inference/build_corpora.py` | Token count in DB matches intended source count exactly. |
| D4 | Ensure transcription source IDs are registered consistently before use (`corpus_gen` path). | `scripts/inference/build_corpora.py` | No unregistered source ID usage. |
| D5 | Replace bare `except:` with explicit exception handling and logging. | `scripts/mechanism/categorize_sections.py` | No bare exception remains in this module. |
| D6 | Replace silent empty-config fallback behavior with explicit warning/error policy. | `src/foundation/config.py` | Missing config files cannot silently alter behavior unnoticed. |
| D7 | Implement or explicitly decommission placeholder/stub analysis methods in human/QC modules. | `src/human/ergonomics.py`, `src/human/page_boundary.py`, `src/foundation/qc/reporting.py` | Placeholders no longer appear in release path. |

### Verification Commands

```bash
rg -n "TODO|NOT COMPUTED|return \\{\\}|except:\\s*$" src scripts
python scripts/inference/build_corpora.py
python scripts/synthesis/run_baseline_assessment.py
```
...
| Task ID | Action | Target Files | Exit Criteria |
|---|---|---|---|
| E1 | Externalize observed anomaly constants and annotate provenance lineage clearly (inputs vs conclusions). | `src/analysis/anomaly/stability_analysis.py`, `src/analysis/anomaly/capacity_bounding.py`, `src/analysis/anomaly/constraint_analysis.py`, config files | No hidden hardcoded observed-value dependence in core anomaly logic. |
| E2 | Implement sensitivity sweep runner and execute documented sweep plan. | `scripts/analysis/run_sensitivity_sweep.py`, `docs/SENSITIVITY_ANALYSIS.md` | Sweep executed; results artifact generated. |
| E3 | Publish sensitivity outcomes and decision impact summary. | `reports/audit/SENSITIVITY_RESULTS.md` | Explicit statement of robustness/fragility across parameter ranges. |

### Verification Commands

```bash
python scripts/analysis/run_sensitivity_sweep.py
test -f reports/audit/SENSITIVITY_RESULTS.md
```
...
| Task ID | Action | Target Files | Exit Criteria |
|---|---|---|---|
| F1 | Replace duplicated runner data-extraction helper with shared query utility usage. | `scripts/human/run_7c_comparative.py`, `src/foundation/core/queries.py` | Single canonical helper path is used. |
| F2 | Normalize exception handling style: no silent swallows in audited script paths. | selected `scripts/` and `src/foundation/*` | Exceptions are explicit and logged. |
| F3 | Standardize script logging conventions (CLI display vs internal logger boundaries). | `scripts/`, `src/foundation/cli/main.py` | Consistent operational behavior and cleaner logs. |
...
| Task ID | Action | Target Files | Exit Criteria |
|---|---|---|---|
| G1 | Add tests for reproducibility pipeline, run identity behavior, provenance writing, and baseline assessment correctness. | `tests/audit/`, `tests/foundation/`, `tests/synthesis/` | New high-risk paths covered. |
| G2 | Add focused tests for anomaly modules currently at 0% coverage. | `tests/analysis/` | Non-zero meaningful coverage in anomaly package. |
| G3 | Address Pydantic config deprecation to remove warning debt. | `src/foundation/runs/context.py` | `pytest` warning removed. |
...
| H2 | Update reproducibility guide with real environment prerequisites and repaired verification flow. | `docs/REPRODUCIBILITY.md` | Following doc commands succeeds in clean setup. |
| H3 | Add explicit provenance/run-output policy docs. | `docs/RUNBOOK.md` or new `docs/PROVENANCE.md` | External readers can trace output lifecycle and identity semantics. |
...
7. A fresh comprehensive audit is generated after remediation:
   - output target: `reports/audit/COMPREHENSIVE_AUDIT_REPORT_5.md`.
