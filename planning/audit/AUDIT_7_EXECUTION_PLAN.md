...
**Source Audit:** `reports/audit/COMPREHENSIVE_AUDIT_REPORT_7.md`  
...
| A4 | Add regression tests for verifier and CI failure semantics (including environment-variable edge cases). | `tests/audit/test_verify_reproduction_contract.py`, optional new CI contract tests | Tests fail if false-positive pass behavior reappears. |
| A5 | Update reproducibility docs to specify expected verifier exit semantics and error behavior. | `docs/REPRODUCIBILITY.md` | Documentation matches actual gate behavior with concrete pass/fail expectations. |
...
python3 -m pytest -q tests/audit/test_verify_reproduction_contract.py
```
...
| Task ID | Action | Target Files | Exit Criteria |
|---|---|---|---|
| B1 | Define explicit sweep modes (`smoke` vs `release`) or equivalent guardrail so bounded runs cannot be mistaken for release evidence. | `scripts/analysis/run_sensitivity_sweep.py` | Release evidence path requires full scenario execution; bounded mode is clearly labeled non-release. |
| B2 | Run canonical full sweep for release dataset profile and regenerate machine/human artifacts. | `status/audit/sensitivity_sweep.json`, `reports/audit/SENSITIVITY_RESULTS.md` | Artifacts reflect full-sweep execution metadata (scenario count and dataset profile consistent with release mode). |
| B3 | Encode release-evidence metadata in output summary (mode, scenario_count_expected vs executed). | `scripts/analysis/run_sensitivity_sweep.py`, artifacts | Consumers can detect whether artifact is release-grade without manual inference. |
| B4 | Update sensitivity documentation to reference latest full-sweep evidence and caveats verbatim. | `docs/SENSITIVITY_ANALYSIS.md` | Documentation and artifacts remain claim-consistent. |
| B5 | Add end-to-end contract test that fails when release report/status are generated from bounded runs. | `tests/analysis/test_sensitivity_sweep_end_to_end.py`, `tests/analysis/test_sensitivity_sweep_guardrails.py` | Regression test enforces release evidence completeness policy. |
...
python3 - <<'PY'
import json
p=json.load(open('status/audit/sensitivity_sweep.json'))
s=p['results']['summary']
print(s['dataset_id'], s['total_scenarios'], s['robustness_decision'])
PY
rg -n "unknown_legacy|sensitivity_sweep_legacy_reconcile|max-scenarios 1" status/audit/sensitivity_sweep.json reports/audit/SENSITIVITY_RESULTS.md docs/SENSITIVITY_ANALYSIS.md
python3 -m pytest -q tests/analysis/test_sensitivity_sweep_end_to_end.py tests/analysis/test_sensitivity_sweep_guardrails.py
```
...
| Task ID | Action | Target Files | Exit Criteria |
|---|---|---|---|
| C1 | Add explicit release-mode switch (or enforce `REQUIRE_COMPUTED=1`) for indistinguishability execution path. | `scripts/synthesis/run_indistinguishability_test.py`, `src/synthesis/profile_extractor.py` | Release mode fails on simulated/fallback profile use. |
| C2 | Add preflight dataset completeness checks for required pharmaceutical pages and required transcription/glyph prerequisites. | `scripts/synthesis/run_indistinguishability_test.py`, optionally helper in `src/synthesis/profile_extractor.py` | Script fails early with actionable missing-data diagnostics instead of deep fallback. |
| C3 | Instrument runtime phases and add explicit timeout/error messaging guidance for long-running segments. | `scripts/synthesis/run_indistinguishability_test.py`, docs | Runtime status is observable; hangs can be diagnosed quickly. |
| C4 | Add regression tests for strict computed mode and missing-data fail-fast behavior. | `tests/synthesis/test_run_indistinguishability_runner.py`, optional new tests | Test suite catches fallback leakage in release path. |
| C5 | Update reproducibility docs to classify this runner as release-evidentiary only under strict computed mode. | `docs/REPRODUCIBILITY.md`, optional `docs/SENSITIVITY_ANALYSIS.md` references | External readers can distinguish exploratory vs release invocation paths. |
...
| Task ID | Action | Target Files | Exit Criteria |
|---|---|---|---|
| D1 | Define policy for stale `runs` rows with missing manifests (e.g., `orphaned`, archived, or purged with audit trail). | `docs/PROVENANCE.md`, `AUDIT_LOG.md` | Clear, documented policy exists for irreparable historical rows. |
| D2 | Extend repair utility to handle missing-manifest rows according to policy and produce a reconciliation report artifact. | `scripts/audit/repair_run_statuses.py` | Utility reports scanned/repaired/orphaned counts deterministically. |
| D3 | Add DB-level tests for missing-manifest handling and idempotent re-run behavior. | `tests/audit/test_repair_run_statuses.py` | Tests enforce no silent partial remediation. |
| D4 | Optionally add one-time migration/archival command for historical stale `status/by_run` verify artifacts or annotate them as legacy. | `scripts/audit/cleanup_status_artifacts.sh`, `docs/PROVENANCE.md` | Legacy artifact handling is explicit and reproducible. |
| D5 | Ensure recent run persistence remains correct for active scripts (`status` final, `timestamp_end` non-null). | `tests/foundation/storage/test_metadata_run_persistence.py`, scripts using `store.save_run(run)` | No new stale rows are generated in normal execution. |
...
con.close()
PY
python3 -m pytest -q tests/audit/test_repair_run_statuses.py tests/foundation/storage/test_metadata_run_persistence.py
```
...
| Task ID | Action | Target Files | Exit Criteria |
|---|---|---|---|
| E1 | Add targeted tests for currently 0% modules or explicitly reclassify non-critical modules outside release gate scope. | `tests/foundation/`, `tests/analysis/`, possible coverage config | 0% module list is reduced or intentionally excluded with documented rationale. |
...
| E5 | Add regression tests covering CI threshold and enforcement behavior. | `tests/audit/` | Gate policy drift is caught automatically. |
...
| Task ID | Action | Target Files | Exit Criteria |
|---|---|---|---|
| F1 | Add pre-release checklist gate/script that verifies intentional diff scope before signoff. | `docs/REPRODUCIBILITY.md`, optional new `scripts/audit/pre_release_check.sh` | Release process includes explicit clean/intended diff confirmation step. |
| F2 | Define retention window and cleanup procedure for `status/by_run` transient verification outputs. | `docs/PROVENANCE.md`, `docs/REPRODUCIBILITY.md`, cleanup script | Teams can consistently clean/retain artifacts with policy traceability. |
| F3 | Ensure cleanup utility supports dry-run reporting + deterministic cleanup summary for audit logs. | `scripts/audit/cleanup_status_artifacts.sh` | Cleanup operations are reviewable and repeatable. |
...
```bash
git status --short
bash scripts/audit/cleanup_status_artifacts.sh list
...
4. `python3 scripts/analysis/run_sensitivity_sweep.py` generates full release-mode canonical artifacts.
5. `REQUIRE_COMPUTED=1 python3 scripts/synthesis/run_indistinguishability_test.py` completes without simulated/fallback metrics.
...
- `reports/audit/FIX_EXECUTION_STATUS_7.md`
- `reports/audit/COMPREHENSIVE_AUDIT_REPORT_8.md` (next assessment-only rerun)
