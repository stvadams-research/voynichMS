...
|---|---|---|---|
| A1 | Define closure taxonomy (for example `FRAMEWORK_CONDITIONAL_CLOSURE`, `DEFERRED_REOPENABLE`, `INCONCLUSIVE`). | `governance/CLOSURE_CONDITIONALITY_POLICY.md` (new) | Taxonomy and examples documented. |
| A2 | Define prohibited contradiction patterns and required closure markers. | `configs/core_skeptic/sk_m1_closure_policy.json` (new) | Machine-readable policy versioned. |
...
python3 - <<'PY'
import json
p=json.load(open('configs/core_skeptic/sk_m1_closure_policy.json'))
print(sorted(p.keys()))
PY
```
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| B1 | Inventory closure claims and classify as conditional vs terminal. | `reports/core_skeptic/SK_M1_CLOSURE_REGISTER.md` (new) | All targeted closure claims listed with status. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| D1 | Create canonical reopening criteria reference document. | `governance/REOPENING_CRITERIA.md` (new) | Single source of truth exists. |
| D2 | Link all closure-bearing docs to canonical criteria source. | phase8_comparative/final closure docs | Every closure doc references canonical criteria. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| E1 | Implement SK-M1 checker for contradictory closure patterns and missing criteria markers. | `scripts/core_skeptic/check_closure_conditionality.py` (new) | Checker exits non-zero on SK-M1 violations. |
| E2 | Add tests for pass/fail/allowlist behavior. | `tests/core_skeptic/test_closure_conditionality_checker.py` (new) | Deterministic policy coverage. |
| E3 | Integrate SK-M1 checker into CI/release paths. | `scripts/ci_check.sh`, `scripts/core_audit/pre_release_check.sh` | Guard runs automatically. |

### Verification

```bash
python3 scripts/core_skeptic/check_closure_conditionality.py
...
6. WS-M1-F (governance/core_audit trace)
...
| R2 | Inconsistent updates across phase8_comparative/final docs create new contradictions. | High | High | Maintain closure register and canonical reopening reference document. |
...
3. Calibrated phase8_comparative/final closure language aligned with reopening criteria.
...
6. SK-M1 execution status report under `reports/core_skeptic/`.
