...
|---|---|---|---|
| A1 | Inventory all closure and non-claim assertions in tracked files and classify strength (qualified vs assertive). | New `reports/core_skeptic/SK_H2_M1_2_ASSERTION_REGISTER.md` | Register captures all tracked assertions and strength tags. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| B1 | Add gate-dependent entitlement sections to claim-boundary policy (`allowed_claim_class_by_gate_state`). | `configs/core_skeptic/sk_h2_claim_language_policy.json`, `governance/CLAIM_BOUNDARY_POLICY.md` | SK-H2 policy can encode degraded entitlement under failed gates. |
| B2 | Add gate-dependent closure constraints (`allowed_closure_class_by_gate_state`). | `configs/core_skeptic/sk_m1_closure_policy.json`, `governance/CLOSURE_CONDITIONALITY_POLICY.md` | SK-M1 policy enforces conditional closure semantics tied to gate health. |
| B3 | Define shared reason-code taxonomy for entitlement downgrades (`GATE_CONTRACT_BLOCKED`, `RELEASE_EVIDENCE_UNQUALIFIED`, etc.). | same + new section in `governance/REOPENING_CRITERIA.md` | Residual states become machine-readable and narrative-stable. |
...
python3 - <<'PY'
import json
h2=json.load(open('configs/core_skeptic/sk_h2_claim_language_policy.json'))
m1=json.load(open('configs/core_skeptic/sk_m1_closure_policy.json'))
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| C1 | Define canonical gate-health artifact schema (`ci_check`, `pre_release_check`, `verify_reproduction`, reason codes, timestamp, mode). | New `core_status/core_audit/release_gate_health_status.json` contract | Gate health can be consumed deterministically by policy checkers. |
| C2 | Implement/define artifact builder from current gate outputs. | New `scripts/core_audit/build_release_gate_health_status.py` | Builder emits deterministic artifact and fail reasons. |
| C3 | Add by-run trace convention for gate-health snapshots. | `core_status/core_audit/by_run/*` | Entitlement decisions are reproducible per run context. |
...
python3 - <<'PY'
import json
p=json.load(open('core_status/core_audit/release_gate_health_status.json'))
...
rg -n "SK-H2.2|SK-M1.2|entitlement|gate health|release_gate_health_status" \
  reports/core_skeptic AUDIT_LOG.md docs
```
...
|---|---|---|---|---|---|
| WS-H2M1.2-A Baseline/Register | COMPLETE | Codex | 2026-02-10 | 2026-02-10 | Added `reports/core_skeptic/SK_H2_M1_2_ASSERTION_REGISTER.md` with assertion/dependency mapping. |
...
7. Combined execution status report under `reports/core_skeptic/`.
