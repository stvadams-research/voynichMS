...
|---|---|---|---|
| A1 | Freeze pass-5 SK-M4 tuple and source evidence references. | New `reports/skeptic/SK_M4_5_BASELINE_REGISTER.md` | Baseline state and evidence map are reproducible. |
...
python3 - <<'PY'
import json
h=json.load(open('status/audit/provenance_health_status.json'))
s=json.load(open('status/audit/provenance_register_sync_status.json'))
g=json.load(open('status/audit/release_gate_health_status.json'))
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| C1 | Add explicit SK-M4 missing-folio non-blocker criterion to skeptic playbook and SK-M4 policy where absent. | `planning/skeptic/ADVERSARIAL_SKEPTIC_PLAYBOOK.md`, SK-M4 policy | Playbook/policy routing is unambiguous. |
...
```bash
rg -n "SK-M4|folio|irrecoverable|non-block" planning/skeptic/ADVERSARIAL_SKEPTIC_PLAYBOOK.md
python3 scripts/skeptic/check_provenance_uncertainty.py --mode ci
```
...
| D2 | Enforce strict parity for status/reason/lane/orphan counts across provenance artifacts. | checker/tests | Cross-artifact mismatches cannot ship silently. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| E1 | Build SK-M4.5 claim-boundary register with lane-specific allowed/disallowed language. | New `reports/skeptic/SK_M4_5_CLAIM_BOUNDARY_REGISTER.md` | Claim scope is explicit and testable. |
| E2 | Synchronize SK-M4-facing docs/reports/registers to SK-M4.5 lane semantics. | README/results/docs/reports/skeptic surfaces | No deterministic certainty language in bounded/qualified lanes. |
...
```bash
rg -n "M4_5_|PROVENANCE_QUALIFIED|bounded|qualified|status/audit/provenance_health_status.json" \
  README.md \
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| F1 | Ensure CI, pre-release, and verify scripts enforce upgraded SK-M4.5 checks consistently. | `scripts/ci_check.sh`, `scripts/audit/pre_release_check.sh`, `scripts/verify_reproduction.sh` | SK-M4 parity checks cannot be bypassed. |
| F2 | Ensure release gate-health dependency snapshot carries decisive SK-M4 lane/reason fields. | `scripts/audit/build_release_gate_health_status.py` | SK-M4 governance state is visible in gate snapshots. |
...
| G2 | Add decision record with fixable vs non-fixable blocker ledger. | New `reports/skeptic/SK_M4_5_DECISION_RECORD.md` | Blocker classification is explicit and stable. |
| G3 | Add execution status report template with command evidence matrix and blocker outcomes. | New `reports/skeptic/SKEPTIC_M4_5_EXECUTION_STATUS.md` | Full pass-5 traceability exists. |
...
1. `reports/skeptic/SK_M4_5_BASELINE_REGISTER.md`
2. `reports/skeptic/SK_M4_5_CLAIM_BOUNDARY_REGISTER.md`
3. `reports/skeptic/SK_M4_5_DECISION_RECORD.md`
4. `reports/skeptic/SKEPTIC_M4_5_EXECUTION_STATUS.md`
