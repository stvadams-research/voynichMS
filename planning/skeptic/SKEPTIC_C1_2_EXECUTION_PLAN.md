...
1. `status/audit/sensitivity_sweep.json` always includes required policy fields:
   - `dataset_policy_pass`
   - `warning_policy_pass`
...
4. `reports/audit/SENSITIVITY_RESULTS.md` is generated from the current canonical JSON artifact and reflects the same values.
5. `scripts/audit/pre_release_check.sh` and `scripts/verify_reproduction.sh` have contract-consistent expectations and deterministic failure messages.
6. Targeted analysis/audit tests prevent regression of the above.
...
|---|---|---|---|
| A1 | Inventory current required sensitivity summary fields used by release/repro gates. | `scripts/audit/pre_release_check.sh`, `scripts/verify_reproduction.sh` | Canonical required-field list documented. |
| A2 | Trace producer path for each required field in sweep runner/report pipeline. | `scripts/analysis/run_sensitivity_sweep.py`, report-render path | Each field has single source assignment point. |
| A3 | Classify current mismatch as stale artifact vs generation regression vs report regression. | `reports/skeptic/SK_C1_2_CONTRACT_REGISTER.md` (new) | Root cause table completed and evidence-linked. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| B1 | Add explicit schema metadata (`schema_version`, `policy_version`, `generated_utc`, `generated_by`). | `status/audit/sensitivity_sweep.json` contract | Artifact carries versioned contract metadata. |
| B2 | Add machine-checkable schema/contract definition. | `configs/audit/sensitivity_artifact_contract.json` (new) | Contract file parseable and test-consumed. |
| B3 | Add contract checker for sensitivity artifact fields and value invariants. | `scripts/audit/check_sensitivity_artifact_contract.py` (new) | Checker fails on missing/invalid required fields. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| C1 | Enforce fail-closed expression for `release_evidence_ready` from complete policy prerequisites only. | `scripts/analysis/run_sensitivity_sweep.py` | No path yields `true` with missing/failed policy fields. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| D1 | Centralize markdown rendering from canonical JSON summary values only. | `reports/audit/SENSITIVITY_RESULTS.md` render path | Report fields mirror artifact values exactly. |
...
rg -n "dataset_policy_pass|warning_policy_pass|warning_density_per_scenario|Total warnings observed|Caveat" \
  reports/audit/SENSITIVITY_RESULTS.md status/audit/sensitivity_sweep.json
```
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| E1 | Wire sensitivity contract checker into CI and/or pre-release sequence. | `scripts/ci_check.sh`, `scripts/audit/pre_release_check.sh` | Contract checker runs automatically. |
| E2 | Extend audit contract tests to assert new checker integration and field expectations. | `tests/audit/test_pre_release_contract.py`, `tests/audit/test_verify_reproduction_contract.py`, `tests/audit/test_ci_check_contract.py` | Contract tests fail on missing integration. |
| E3 | Extend sensitivity guardrail/E2E tests for missing-field, empty-caveat, and stale-schema cases. | `tests/analysis/test_sensitivity_sweep_guardrails.py`, `tests/analysis/test_sensitivity_sweep_end_to_end.py` | Regression matrix covers SK-C1.2 failure modes. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| F1 | Update sensitivity docs with exact release-field contract and caveat rules. | `docs/SENSITIVITY_ANALYSIS.md` | Field-level policy semantics documented. |
| F2 | Update reproducibility doc with SK-C1.2 contract check command path. | `docs/REPRODUCIBILITY.md` | Repro flow includes contract checker and expected fields. |
| F3 | Add SK-C1.2 execution status and audit log trace templates. | `reports/skeptic/SKEPTIC_C1_2_EXECUTION_STATUS.md` (during execution), `AUDIT_LOG.md` | End-to-end traceability path prepared. |
...
6. WS-C1.2-F (docs/audit traceability)
...
|---|---|---|---|---|---|
| WS-C1.2-A Baseline/Root Cause | COMPLETE | Codex | 2026-02-10 | 2026-02-10 | Added root-cause register: `reports/skeptic/SK_C1_2_CONTRACT_REGISTER.md`. |
...
7. SK-C1.2 execution status report under `reports/skeptic/`.
