...
- `results/reports/PHASE_4_RESULTS.md` (final determination + stale pending table).
...
- Governance/audit traceability for SK-M3 coherence decisions.
...
|---|---|---|---|
| A1 | Define status taxonomy (for example `COMPLETE`, `ARCHIVAL_SNAPSHOT`, `SUPERSEDED`, `IN_PROGRESS`). | `docs/REPORT_COHERENCE_POLICY.md` (new) | Status classes and examples documented. |
| A2 | Define contradiction patterns (final determination + unresolved pending rows) and allowed exceptions. | `configs/skeptic/sk_m3_report_coherence_policy.json` (new) | Machine-readable policy created and versioned. |
...
python3 - <<'PY'
import json
p=json.load(open('configs/skeptic/sk_m3_report_coherence_policy.json'))
print(sorted(p.keys()))
PY
```
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| B1 | Inventory status-bearing tables/sections in tracked Phase 4 artifacts. | `reports/skeptic/SK_M3_COHERENCE_REGISTER.md` (new) | All tracked status blocks listed. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| C1 | Remediate contradictory residual status table in `PHASE_4_RESULTS.md`. | `results/reports/PHASE_4_RESULTS.md` | No unresolved stale `PENDING` rows conflict with final determination. |
| C2 | Harmonize status language between Phase 4 result/conclusion artifacts. | `results/reports/PHASE_4_CONCLUSIONS.md`, `results/reports/PHASE_4_5_METHOD_CONDITION_MAP.md` | Method-state framing is consistent across docs. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| D1 | Define canonical Phase 4 method status schema. | `results/reports/PHASE_4_STATUS_INDEX.json` (new) | Schema + status rows are present and parseable. |
...
| D3 | Link docs to canonical status index to reduce table drift. | `results/reports/PHASE_4_RESULTS.md`, related docs | Docs reference status index as source of truth. |
...
python3 - <<'PY'
import json
d=json.load(open('results/reports/PHASE_4_STATUS_INDEX.json'))
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| E1 | Implement SK-M3 checker for contradictory status patterns and required coherence markers. | `scripts/skeptic/check_report_coherence.py` (new) | Checker exits non-zero on SK-M3 violations. |
| E2 | Add SK-M3 checker tests (pass/fail/allowlist/artifact-schema). | `tests/skeptic/test_report_coherence_checker.py` (new) | Deterministic policy coverage exists. |
| E3 | Integrate SK-M3 checker into CI/release scripts. | `scripts/ci_check.sh`, `scripts/audit/pre_release_check.sh` | Guard runs automatically in ci/release modes. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| F1 | Add SK-M3 section to reproducibility/governance docs with coherence verification commands. | `docs/REPRODUCIBILITY.md` | SK-M3 verification path documented. |
| F2 | Add SK-M3 execution status report template for remediation pass. | `reports/skeptic/SKEPTIC_M3_EXECUTION_STATUS.md` (during execution) | Full change/test trace recorded. |
...
3. Remediated Phase 4 status/coherence language across tracked reports.
4. Canonical Phase 4 method-status index artifact.
5. SK-M3 checker and tests integrated into CI/release checks.
6. SK-M3 execution status report under `reports/skeptic/`.
