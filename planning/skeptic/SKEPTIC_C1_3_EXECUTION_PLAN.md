...
- `status/audit/release_gate_health_status.json` remains:
...
|---|---|---|---|
| A1 | Freeze current sensitivity summary and release-gate health snapshots used for SK-C1.3 remediation. | `status/audit/sensitivity_sweep.json`, `status/audit/release_gate_health_status.json` | Baseline snapshot IDs/timestamps captured. |
| A2 | Create structured SK-C1.3 gap register mapping each failing field to producer, consumer, and policy clause. | New `reports/skeptic/SK_C1_3_CONTRACT_REGISTER.md` | Every failing field has an owner and closure test. |
...
python3 - <<'PY'
import json
s=json.load(open('status/audit/sensitivity_sweep.json'))['results']['summary']
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| B1 | Audit release-mode invocation path and defaults in `run_sensitivity_sweep.py` and wrapper scripts. | `scripts/analysis/run_sensitivity_sweep.py` | Release execution path is explicit and deterministic. |
| B2 | Enforce artifact provenance markers that distinguish `release` vs `iterative` generation and block ambiguity. | `status/audit/sensitivity_sweep.json` | Artifact includes unambiguous generation mode provenance. |
| B3 | Add stale-artifact guard in release consumers to prevent old iterative artifacts being treated as release candidates. | `scripts/audit/pre_release_check.sh`, `scripts/verify_reproduction.sh` | Release consumers reject stale/mode-mismatched artifacts with explicit reason. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| C1 | Review and harden dataset policy thresholds/IDs for release evidence to avoid permissive leakage. | `configs/audit/release_evidence_policy.json` | Policy is explicit, versioned, and test-backed. |
| C2 | Reconcile robustness decision logic so `INCONCLUSIVE` always blocks release readiness and closure language. | `scripts/analysis/run_sensitivity_sweep.py` | `INCONCLUSIVE` cannot coexist with release-ready state. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| D1 | Validate warning-family classification coverage for current warning streams and fallback types. | `scripts/analysis/run_sensitivity_sweep.py` | Warning burden metrics are complete and stable. |
...
| D3 | Enforce caveat presence and report consistency when `total_warning_count>0` or warning policy fails. | `reports/audit/SENSITIVITY_RESULTS.md` renderer + checker | No warning-bearing artifact can emit caveat-empty narrative. |

### Verification

```bash
python3 scripts/audit/check_sensitivity_artifact_contract.py --mode release
rg -n "warning|caveat|release_evidence_ready|warning_policy_pass" reports/audit/SENSITIVITY_RESULTS.md
```
...
| E2 | Ensure pre-release and verify scripts share one canonical sensitivity contract checker path and reason-code semantics. | `scripts/audit/pre_release_check.sh`, `scripts/verify_reproduction.sh` | Same failing condition yields same reason and remediation message. |
| E3 | Validate gate-health builder consumes updated sensitivity outcomes and reason-code transitions deterministically. | `scripts/audit/build_release_gate_health_status.py` | Gate-health status changes only when sensitivity contract truly changes. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| F1 | Extend sensitivity guardrail and e2e tests for pass-3 failure tuple (`iterative`, non-ready, policy false, inconclusive). | `tests/analysis/test_sensitivity_sweep_guardrails.py`, `tests/analysis/test_sensitivity_sweep_end_to_end.py` | Pass-3 failure shape has explicit regression tests. |
| F2 | Extend audit contract tests to assert aligned sensitivity checks across pre-release/verify/CI stages. | `tests/audit/test_pre_release_contract.py`, `tests/audit/test_verify_reproduction_contract.py`, `tests/audit/test_ci_check_contract.py` | Nested verification mismatch is test-covered. |
| F3 | Add checker tests for reason-code parity between contract checker and gate-health builder. | `tests/audit/test_release_gate_health_status_builder.py` + new cases | Reason-code drift is blocked by tests. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| G1 | Update sensitivity and reproducibility docs to reflect exact release-readiness contract for pass-3 closure. | `docs/SENSITIVITY_ANALYSIS.md`, `docs/REPRODUCIBILITY.md` | Docs match script/checker behavior exactly. |
| G2 | Produce SK-C1.3 execution status template/report for future execution pass. | `reports/skeptic/SKEPTIC_C1_3_EXECUTION_STATUS.md` (during execution) | Status path prepared before implementation. |
...
6. SK-C1.3 execution status report (`reports/skeptic/SKEPTIC_C1_3_EXECUTION_STATUS.md`).
