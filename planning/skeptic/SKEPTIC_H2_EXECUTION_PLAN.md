...
- Bounded non-claims are present in `results/reports/PHASE_5_FINAL_FINDINGS_SUMMARY.md` (`what this does not claim` section).
- Public-facing statements still include categorical language:
  - `README.md` ("Language Hypothesis Falsified")
  - `results/reports/PHASE_4_5_CLOSURE_STATEMENT.md` ("PROJECT TERMINATED (Exhaustive)", "scientifically unjustified")
...
|---|---|---|---|
| A1 | Define claim-strength taxonomy (`CONCLUSIVE_WITHIN_SCOPE`, `QUALIFIED`, `INCONCLUSIVE`, `DEFERRED`). | `docs/CLAIM_BOUNDARY_POLICY.md` (new) | Taxonomy and examples documented. |
| A2 | Define policy exceptions and prohibited phrasing list (for public docs). | `configs/skeptic/sk_h2_claim_language_policy.json` (new) | Policy machine-readable and versioned. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| B1 | Enumerate claims in priority docs (README, closure statements, final reports). | `reports/skeptic/SK_H2_CLAIM_REGISTER.md` (new) | All target claims listed with severity tags. |
...
| C2 | Reframe closure statement from absolute terminal language to conditional framework-bounded closure. | `results/reports/PHASE_4_5_CLOSURE_STATEMENT.md` | Reopening criteria and scope are first-class, not contradictory. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| D1 | Define required conclusion block template (claim, scope, evidence links, non-claim boundary). | `docs/CLAIM_BOUNDARY_POLICY.md` | Template is documented and reused. |
| D2 | Apply template to top-level conclusion documents. | `README.md`, `results/reports/PHASE_4_5_CLOSURE_STATEMENT.md`, `results/reports/FINAL_PHASE_3_3_REPORT.md` | Conclusion sections use uniform schema. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| E1 | Add claim-policy checker script (pattern and boundary checks). | `scripts/skeptic/check_claim_boundaries.py` (new) | Script exits non-zero on violations. |
| E2 | Add tests for checker and known policy violations/allowances. | `tests/skeptic/test_claim_boundary_checker.py` (new) | Deterministic coverage for pass/fail paths. |
| E3 | Integrate checker in CI or pre-release path. | `scripts/ci_check.sh` and/or `scripts/audit/pre_release_check.sh` | Guard runs automatically in verification path. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| F1 | Update reproducibility/governance docs with claim-boundary policy references. | `docs/REPRODUCIBILITY.md`, relevant governance docs | Contributor path includes policy usage. |
| F2 | Add skeptic-execution summary documenting SK-H2 remediation rationale. | `reports/skeptic/SKEPTIC_H2_EXECUTION_STATUS.md` (during execution) | Full change log and residual risks recorded. |
...
6. WS-H2-F (docs/governance traceability)
...
5. Execution status report under `reports/skeptic/`.
