...
- Control-comparability artifact remains blocked:
  - `status/synthesis/CONTROL_COMPARABILITY_STATUS.json` -> `status=NON_COMPARABLE_BLOCKED`
  - `status/synthesis/CONTROL_COMPARABILITY_STATUS.json` -> `reason_code=DATA_AVAILABILITY`
...
|---|---|---|---|
| A1 | Record current SK-H3 artifact state (`CONTROL_COMPARABILITY_STATUS`, `TURING_TEST_RESULTS`). | `status/synthesis/CONTROL_COMPARABILITY_STATUS.json`, `status/synthesis/TURING_TEST_RESULTS.json` | Baseline snapshot captured. |
| A2 | Trace strict preflight data-availability block details and affected page set. | `status/synthesis/TURING_TEST_RESULTS.json` (`preflight.*`) | Missing page inventory canonicalized. |
| A3 | Create residual root-cause register for SK-H3.2. | `reports/skeptic/SK_H3_2_DATA_AVAILABILITY_REGISTER.md` (new) | Root-cause matrix completed with evidence links. |
...
python3 - <<'PY'
import json
for p in ['status/synthesis/CONTROL_COMPARABILITY_STATUS.json','status/synthesis/TURING_TEST_RESULTS.json']:
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| B1 | Add machine-readable SK-H3 data-availability policy (approved reason codes, required evidence fields, lost-page handling). | `configs/skeptic/sk_h3_data_availability_policy.json` (new) | Policy is parseable and versioned. |
| B2 | Emit canonical data-availability status artifact for SK-H3 decisions. | `status/synthesis/CONTROL_COMPARABILITY_DATA_AVAILABILITY.json` (new) | Artifact contains expected/available/missing sets and policy pass flags. |
| B3 | Add checker for SK-H3 data-availability contract consistency. | `scripts/skeptic/check_control_data_availability.py` (new) | Checker fails on missing/contradictory blocker evidence. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| C1 | Implement available-data comparability runner path with explicit lane labeling. | `scripts/synthesis/run_control_matching_audit.py` and/or new `scripts/synthesis/run_control_comparability_available.py` | Lane emits explicit `evidence_scope=available_subset`. |
| C2 | Add status field separating full-data closure from subset-qualified comparability. | `status/synthesis/CONTROL_COMPARABILITY_STATUS.json` | Status encodes scope and closure eligibility. |
...
python3 - <<'PY'
import json
r=json.load(open('status/synthesis/CONTROL_COMPARABILITY_STATUS.json'))['results']
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| D1 | Reconcile SK-H3 checker + release/repro gates with explicit handling for approved `DATA_AVAILABILITY` block states. | `scripts/skeptic/check_control_comparability.py`, `scripts/audit/pre_release_check.sh`, `scripts/verify_reproduction.sh` | Gates fail only on unapproved/incoherent states; approved blocked states remain bounded. |
| D2 | Add deterministic failure messages for invalid block evidence (missing pages not reported, inconsistent status/reason combinations). | same | Failure reasons are actionable and reproducible. |
| D3 | Add contract tests for approved blocked vs disallowed blocked states. | `tests/skeptic/test_control_comparability_checker.py`, `tests/audit/test_pre_release_contract.py`, `tests/audit/test_verify_reproduction_contract.py` | Regression matrix covers status semantic drift. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| E1 | Align comparative/control narrative docs with SK-H3 residual status semantics. | `docs/GENERATOR_MATCHING.md`, `docs/METHODS_REFERENCE.md`, `docs/CONTROL_COMPARABILITY_POLICY.md` | Language reflects bounded claims for blocked/qualified states. |
| E2 | Update reproducibility/runbook with explicit SK-H3 residual verification flow. | `docs/REPRODUCIBILITY.md`, `docs/RUNBOOK.md` | Operators can reproduce both block and subset-qualified checks. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| F1 | Add SK-H3.2 execution status report template/artifact. | `reports/skeptic/SKEPTIC_H3_2_EXECUTION_STATUS.md` (during execution) | Implementation and verification evidence documented. |
...
| F3 | Update plan status tracker and final decision matrix outcome. | `planning/skeptic/SKEPTIC_H3_2_EXECUTION_PLAN.md` | Workstream status and final outcome explicit. |

### Verification

```bash
rg -n "SK-H3.2|DATA_AVAILABILITY|evidence_scope|allowed_claim" AUDIT_LOG.md reports/skeptic planning/skeptic/SKEPTIC_H3_2_EXECUTION_PLAN.md
```
...
4. WS-H3.2-D (status/gate reconciliation)
...
|---|---|---|---|---|---|
| WS-H3.2-A Baseline/Root Cause | COMPLETE | Codex | 2026-02-10 | 2026-02-10 | Added residual root-cause register: `reports/skeptic/SK_H3_2_DATA_AVAILABILITY_REGISTER.md`. |
...
7. SK-H3.2 execution status report under `reports/skeptic/`.
