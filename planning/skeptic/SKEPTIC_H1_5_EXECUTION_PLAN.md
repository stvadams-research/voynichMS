...
- multimodal status/robustness/checker contract incoherence,
...
|---|---|---|---|
| A1 | Freeze canonical SK-H1 pass-5 tuple (status, lane, residual, robustness counts). | New `reports/skeptic/SK_H1_5_BASELINE_REGISTER.md` | Baseline frozen with run_id/timestamp and residual vectors. |
| A2 | Produce feasibility proof for current robust criteria against current lane registry. | New `reports/skeptic/SK_H1_5_FEASIBILITY_REGISTER.md` | Explicit `reachable` vs `unreachable` verdict with formal rationale. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| B1 | Introduce lane classes: `entitlement`, `diagnostic`, `stress`. | `configs/skeptic/sk_h1_multimodal_policy.json` | Lane registry explicitly typed and versioned. |
...
python3 scripts/mechanism/run_5i_anchor_coupling.py --seed 31415 --max-lines-per-cohort 1600
```
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| D1 | Add explicit H1.5 lane field (`h1_5_closure_lane`) and residual reason. | `src/mechanism/anchor_coupling.py`, `scripts/mechanism/run_5i_anchor_coupling.py` | H1.5 class is emitted deterministically. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| E1 | Extend H1 status policy for H1.5 lane classes and entitlement lane rules. | `configs/skeptic/sk_h1_multimodal_status_policy.json` | Lane mapping is explicit and machine-enforced. |
| E2 | Update multimodal checker for H1.5 contracts. | `scripts/skeptic/check_multimodal_coupling.py` | Checker rejects inconsistent entitlement/diagnostic semantics. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| F1 | Add H1.5 lane and feasibility fields to gate-health dependency snapshot. | `scripts/audit/build_release_gate_health_status.py` | Operational status surfaces H1.5 class deterministically. |
| F2 | Update claim-boundary register for H1.5 lane classes. | New `reports/skeptic/SK_H1_5_CLAIM_BOUNDARY_REGISTER.md` | Overreach detection remains explicit. |
| F3 | Update Phase 5/7 report wording for new lane semantics. | `results/reports/PHASE_5H_RESULTS.md`, `results/reports/PHASE_5I_RESULTS.md`, `results/reports/PHASE_5_FINAL_FINDINGS_SUMMARY.md`, `reports/human/PHASE_7_FINDINGS_SUMMARY.md`, `reports/human/PHASE_7B_RESULTS.md` | Narrative entitlement matches machine lane state. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| G1 | Add tests proving robust closure feasibility classification behavior. | `tests/mechanism/test_anchor_coupling.py`, `tests/mechanism/test_anchor_coupling_contract.py` | “Unreachable by construction” is test-detectable. |
| G2 | Add checker tests for entitlement-vs-diagnostic lane separation. | `tests/skeptic/test_multimodal_coupling_checker.py` | Misclassified diagnostic lanes fail. |
| G3 | Add contract tests for H1.5 markers in CI/pre-release/verify scripts and gate-health builder. | `tests/audit/test_ci_check_contract.py`, `tests/audit/test_pre_release_contract.py`, `tests/audit/test_verify_reproduction_contract.py`, `tests/audit/test_release_gate_health_status_builder.py` | Gate/report drift blocked in CI. |
| G4 | Add claim-boundary tests for H1.5 lane wording constraints. | `tests/human/test_phase7_claim_guardrails.py` | Over-claim patterns fail closed. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| H1 | Create H1.5 decision record with final lane assignment and rationale. | New `reports/skeptic/SK_H1_5_DECISION_RECORD.md` | Final disposition is explicit and auditable. |
| H2 | Create H1.5 execution status report for implementation pass. | New `reports/skeptic/SKEPTIC_H1_5_EXECUTION_STATUS.md` | Command evidence and outcomes are traceable. |
...
rg -n "SK-H1.5|H1_5_|reopen|blocker|bounded|entitlement" \
  reports/skeptic \
  AUDIT_LOG.md \
  planning/skeptic/SKEPTIC_H1_5_EXECUTION_PLAN.md
```
...
| WS-H1.5-G Regression Locking | COMPLETE | Codex | 2026-02-10 | 2026-02-10 | Targeted mechanism/skeptic/audit/human test suite passes with H1.5 contracts. |
...
1. `reports/skeptic/SK_H1_5_BASELINE_REGISTER.md`
2. `reports/skeptic/SK_H1_5_FEASIBILITY_REGISTER.md`
3. updated H1 policy/checker/producer artifacts implementing entitlement-vs-diagnostic lane semantics
4. `reports/skeptic/SK_H1_5_CLAIM_BOUNDARY_REGISTER.md`
5. expanded H1.5 regression tests (mechanism/checker/audit/human surfaces)
6. `reports/skeptic/SK_H1_5_DECISION_RECORD.md`
7. `reports/skeptic/SKEPTIC_H1_5_EXECUTION_STATUS.md`
