...
- `results/reports/PHASE_5H_RESULTS.md` reports near-total anchor imbalance and explicitly notes geometry needs refinement.
- `results/reports/PHASE_5I_RESULTS.md` marks layout/illustration coupling as preliminary.
- `reports/human/PHASE_7_FINDINGS_SUMMARY.md` states no significant adaptation to illustration proximity as if decisively settled.
...
|---|---|---|---|
| A1 | Define multimodal status taxonomy and escalation rules. | `docs/SENSITIVITY_ANALYSIS.md` or new `docs/MULTIMODAL_COUPLING_POLICY.md` | Status classes and claim limits documented. |
| A2 | Add policy config for adequacy thresholds and significance criteria. | `configs/skeptic/sk_h1_multimodal_policy.json` (new) | Script reads policy; defaults are explicit and versioned. |
| A3 | Define canonical output contract for multimodal result artifact. | `results/mechanism/anchor_coupling_confirmatory.json` (new canonical) | Artifact contains status, adequacy diagnostics, and inference fields. |
...
python3 - <<'PY'
import json
cfg=json.load(open('configs/skeptic/sk_h1_multimodal_policy.json'))
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| B1 | Extend anchor generation to support explicit method variants (thresholds/modes) with provenance tags. | `scripts/mechanism/generate_all_anchors.py` | Multiple anchor methods can be generated/referenced deterministically. |
| B2 | Add anchor coverage audit utility (token coverage, page coverage, cohort ratio). | `scripts/mechanism/audit_anchor_coverage.py` (new) | Coverage report produced before coupling analysis. |
| B3 | Replace binary anchored/unanchored partition with distance/region-aware cohort builder and matched sampling option. | `scripts/mechanism/run_5i_anchor_coupling.py` or new shared module under `src/mechanism` | Cohorts satisfy adequacy thresholds or fail with explicit status. |
| B4 | Persist adequacy diagnostics (counts, ratios, pages, recurring contexts) to artifact and report table. | `results/mechanism/anchor_coupling_confirmatory.json`, `results/reports/PHASE_5I_RESULTS.md` | Readers can see why status is conclusive vs inconclusive. |

### Verification

```bash
python3 scripts/mechanism/generate_all_anchors.py --threshold 0.05
python3 scripts/mechanism/generate_all_anchors.py --threshold 0.10
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| C1 | Add bootstrap CI for anchored-vs-unanchored consistency delta. | `scripts/mechanism/run_5i_anchor_coupling.py` | Artifact includes `delta_ci_low`, `delta_ci_high`. |
...
| C5 | Publish confirmatory report section with explicit non-claims. | `results/reports/PHASE_5I_RESULTS.md` and `results/reports/PHASE_5_FINAL_FINDINGS_SUMMARY.md` | Multimodal section references status artifact and uncertainty. |
...
python3 - <<'PY'
import json
d=json.load(open('results/mechanism/anchor_coupling_confirmatory.json'))
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| D1 | Add explicit multimodal-status ingestion into Phase 7B/summary assembly path. | `scripts/human/run_7b_codicology.py`, `reports/human/PHASE_7_FINDINGS_SUMMARY.md` generation flow | Phase 7 summary states conclusive/inconclusive status correctly. |
| D2 | Remove unconditional wording like "no significant adaptation" when status is inconclusive or blocked. | `reports/human/PHASE_7_FINDINGS_SUMMARY.md` | Claim text becomes conditional and evidence-bounded. |
...
rg -n "illustration|coupling|inconclusive|conclusive" reports/human/PHASE_7_FINDINGS_SUMMARY.md
```
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| E1 | Reclassify `PHASE_5H_RESULTS.md` as pilot/inconclusive where adequacy fails. | `results/reports/PHASE_5H_RESULTS.md` | Report no longer claims decisive proximity outcome from underpowered split. |
| E2 | Align `PHASE_5I_RESULTS.md` conclusion with confirmatory status artifact. | `results/reports/PHASE_5I_RESULTS.md` | Conclusion includes explicit confidence/uncertainty language. |
| E3 | Update final Phase 5 synthesis section to prevent overgeneralization. | `results/reports/PHASE_5_FINAL_FINDINGS_SUMMARY.md` | Multimodal paragraph reflects status class and constraints. |
| E4 | Add skeptic-facing trace note describing SK-H1 closure logic and remaining unknowns. | `reports/skeptic/SKEPTIC_H1_EXECUTION_STATUS.md` (during execution) | Closure rationale is auditable. |
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| F1 | Add unit tests for cohort adequacy checks and status mapping. | `tests/mechanism/test_anchor_coupling.py` (new) | Underpowered inputs map to inconclusive/block states. |
| F2 | Add script-level contract tests for mandatory output fields. | `tests/mechanism/test_anchor_coupling_contract.py` (new) | Missing CI/p-value/status fields fail tests. |
| F3 | Add report-language guard tests to prevent categorical claims on inconclusive artifacts. | `tests/human/test_phase7_claim_guardrails.py` (new) | Text-generation/report tests fail if wording exceeds evidence class. |
...
```bash
python3 -m pytest -q tests/mechanism/test_anchor_coupling.py \
...
| ID | Task | Target Artifacts | Completion Signal |
|---|---|---|---|
| G1 | Add reproducibility section for anchor-method generation and confirmatory run sequence. | `docs/REPRODUCIBILITY.md` | Operator can run end-to-end multimodal pipeline from docs. |
| G2 | Document runtime progress/status outputs for long runs. | `docs/REPRODUCIBILITY.md`, script console/progress behavior | Operators can distinguish long-running vs stalled states. |
...
| WS-H1-F Tests/Contracts | COMPLETE | Codex | 2026-02-10 | 2026-02-10 | Added mechanism/human guardrail tests and validated targeted suites. |
...
6. Execution summary in `reports/skeptic/SKEPTIC_H1_EXECUTION_STATUS.md`.
