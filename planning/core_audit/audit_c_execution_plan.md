...
- **Action:** Modify `src/phase3_synthesis/generators/grammar_based.py` to accept a seed or utilize the global `RandomnessController`.
- **Implementation:** Replace `random.choices()` calls with `random.Random(seed).choices()`.

### C3: Feature Discovery Seeding
- **Action:** Audit all 12+ `random.uniform()` calls in `src/phase3_synthesis/refinement/feature_discovery.py`.
...
- **Action:** Fix non-deterministic scrambled control generation in `src/phase3_synthesis/indistinguishability.py`.
...
- **Action:** Update all 8+ files in `src/phase5_mechanism/*/simulators.py` to ensure local RNG state.
...
- **Implementation:** Move these to a new `configs/phase6_functional/model_params.json` or update `src/phase1_foundation/core/config.py`.
...
- **Action:** Create `governance/CONFIG_REFERENCE.md`.
