...
**Source:** `reports/core_audit/COMPREHENSIVE_AUDIT_REPORT_3.md`
...
| B: Threshold Externalization | COMPLETE | `configs/phase2_analysis/thresholds.json` added and wired into consuming modules. |
...
**Files:**
- `src/phase1_foundation/controls/self_citation.py:19-48` (C3) — Replace `random.seed(seed)` + 8 bare `random.*()` calls with `rng = random.Random(seed)` instance. Thread `rng` through all internal calls.
- `src/phase1_foundation/controls/synthetic.py:13` (C4, CS1) — Same pattern: replace `random.seed(seed)` with local `rng` instance.
- `src/phase1_foundation/controls/table_grille.py:18-56` — Same pattern (discovered in agent scan, not separately numbered). Replace `random.seed(seed)` + `random.choice/randint` with `rng` instance.
- `src/phase1_foundation/controls/mechanical_reuse.py:20,44` — Same pattern. Replace global seed with local instance.
...
- `src/phase3_synthesis/text_generator.py:308` (H5) — Currently uses `random.Random(seed)` directly. Wire through `RandomnessController.seeded_context()` or at minimum document why controller is bypassed.
- `src/phase1_foundation/regions/dummy.py:59` (R8) — Uses `random.Random(self.seed)` correctly; add comment noting controller bypass is intentional for dummy data.
- 8+ files in `src/phase5_mechanism/*/simulators.py` (R7) — All correctly seeded with `random.Random(seed)`. Low priority to convert to controller, but add comment explaining pattern.

### A5: Remove `uuid.uuid4()` non-determinism

- `src/phase1_foundation/controls/synthetic.py:2` (R9) — If UUID is used for record IDs, replace with `DeterministicIDFactory`. If used for non-critical labels, document why non-determinism is acceptable.
...
- Run `python -m pytest tests/core_audit/test_determinism.py` to verify seeded reproducibility.
...
Create `configs/phase2_analysis/thresholds.json` with sections:
```
{
  "mapping_stability": {
...
**Files to update:**
1. `src/phase2_analysis/stress_tests/mapping_stability.py:169,343-393` (C8, H7) — Load from config; replace 8+ hardcoded values.
2. `src/phase2_analysis/stress_tests/locality.py:176-183,315-322,389-399,579-584,655-657` (C9, C10, H19, H20, H21) — Load from config; replace 12+ hardcoded values.
3. `src/phase3_synthesis/indistinguishability.py:79-80` (H8) — Load separation thresholds.
4. `src/phase1_foundation/phase2_analysis/comparator.py:33-36` (H9) — Load comparison thresholds.
5. `src/phase2_analysis/anomaly/stability_analysis.py:228-247` (H13) — Load sensitivity thresholds.
6. `src/phase3_synthesis/refinement/constraint_formalization.py:51,58` (H22) — Load feature thresholds.
...
**File:** `src/phase2_analysis/anomaly/capacity_bounding.py:39-44`
...
**File:** `src/phase2_analysis/anomaly/constraint_analysis.py:79-207`
...
**File:** `src/phase3_synthesis/refinement/feature_discovery.py` (CL5)
...
- `governance/governance/METHODS_REFERENCE.md` updated with circularity disclosure section.
...
**File:** `src/phase3_synthesis/indistinguishability.py:120` (C11)
...
**File:** `src/phase3_synthesis/profile_extractor.py:223,236,248` (H10)
...
**File:** `src/phase3_synthesis/refinement/feature_discovery.py:111,156,203,270,620,677,726` (H9)
...
**File:** `src/phase3_synthesis/text_generator.py:325,330` (P6)
...
| `phase2_analysis/sensitivity.py:36` (P7) | `print(f"Error in ...")` | `logger.error("Sensitivity sweep error for %s=%s: %s", param_name, val, e)` |

### E4: Handle `float("nan")` propagation

**File:** `src/phase3_synthesis/refinement/feature_discovery.py` (S5)
...
**File:** `src/phase1_foundation/metrics/library.py:88-103` (H1, MR1, TD3)
...
**File:** `src/phase1_foundation/metrics/library.py:156-289` (MR2)

1. Add `computation_path` field to `MetricResult.details` (already has `method` key - verify it's always set).
2. In `governance/governance/METHODS_REFERENCE.md`, document that embedding-based and bbox-based paths produce different scales and are NOT directly comparable.
...
**File:** `src/phase1_foundation/controls/synthetic.py:38-43` (H7, CS2)
...
2. In `governance/governance/METHODS_REFERENCE.md`, add a "Control Pipeline" section documenting the asymmetry and its justification.
...
**File:** `src/phase2_analysis/stress_tests/interface.py:25-47` (OV1)
...
Create `src/phase1_foundation/core/queries.py` additions (or new `src/phase1_foundation/script_helpers.py`):
...
**File:** `src/phase1_foundation/alignment/engine.py:46-73` (H15, TD1)
...
**Files:** `core/ids.py`, `phase7_human/quire_analysis.py`, `phase3_synthesis/interface.py` (TD2)

1. Add to `governance/GLOSSARY.md`: "Page: abstract database record. Folio: physical manuscript leaf (e.g., f1r, f2v). In this codebase, PageRecord.id stores folio identifiers."
...
**Scope:** Documentation only (TD4). Add to `governance/GLOSSARY.md`:
...
### Verification
- `governance/GLOSSARY.md` covers all four terms.
...
**Option A (preferred):** Create `scripts/phase2_analysis/run_sensitivity_sweep.py` implementing the documented plan:
...
5. Store results in `reports/core_audit/SENSITIVITY_RESULTS.md`.

**Option B (minimum):** Add a prominent "STATUS: NOT YET EXECUTED" banner to `governance/SENSITIVITY_ANALYSIS.md` so readers know results are unavailable.

### K2: Update governance/REPRODUCIBILITY.md for Phase 4-5

**File:** `governance/governance/REPRODUCIBILITY.md` (H18)
...
### K4: Update governance/METHODS_REFERENCE.md

**File:** `governance/governance/METHODS_REFERENCE.md`
...
Create `tests/phase3_synthesis/test_grammar_based.py`:
...
Create `tests/phase1_foundation/test_controls.py`:
...
Extend `tests/phase1_foundation/test_boundary_cases.py`:
...
Create `tests/phase7_human/test_quire_analysis.py`:
- Test `get_quire()` with valid folios.
- Test with malformed folio IDs (after Workstream E narrows exceptions).

### Verification
- `python -m pytest tests/ -v` passes.
- Coverage for phase3_synthesis/, phase5_mechanism/, and phase7_human/ modules > 0%.
...
**File:** `src/phase1_foundation/core/provenance.py:13` (IO1)
...
**File:** `src/phase1_foundation/core/randomness.py:130` (IO2)
...
**Files:** `phase3_synthesis/text_generator.py`, `alignment/engine.py`, `refinement/feature_discovery.py` (IO3)

Add `-> List[str]`, `-> None`, `-> Dict[str, float]` etc. to the 8+ methods identified.

### Verification
- `mypy src/phase1_foundation/core/provenance.py` passes.
