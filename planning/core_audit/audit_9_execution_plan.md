...
- Legacy `core_status/by_run` verify artifact drift (currently clean in workspace)
...
| Task ID | Action | Target Files | Exit Criteria |
|---|---|---|---|
| B1 | Define canonical page-ID normalization policy (e.g., `f89r1`/`f89r2` -> base `f89r`) for preflight matching. | `scripts/phase3_synthesis/run_indistinguishability_test.py`, `src/phase3_synthesis/profile_extractor.py` | Policy is explicit and deterministic. |
| B2 | Implement normalization in preflight availability checks and emitted diagnostics. | `scripts/phase3_synthesis/run_indistinguishability_test.py` | Missing-page list no longer over-counts split-folio cases. |
| B3 | Add tests covering canonical, split, and genuinely missing folio scenarios. | `tests/phase3_synthesis/test_run_indistinguishability_runner.py` | Tests fail on regression to exact-string-only matching. |
| B4 | Validate by-run artifact payload correctness for missing page counts and lists. | `core_status/phase3_synthesis/TURING_TEST_RESULTS.json` (runtime output) | Preflight payload reflects true missing set under normalization rules. |
...
```bash
rg -n "data availability|lost|unavailable source|strict" docs planning/core_audit reports/core_audit
python3 scripts/phase3_synthesis/run_indistinguishability_test.py --preflight-only
```
...
| Task ID | Action | Target Files | Exit Criteria |
|---|---|---|---|
| D1 | Decide release-policy default: strict-by-default or mandatory strict flag for release scripts. | `scripts/verify_reproduction.sh`, `scripts/core_audit/pre_release_check.sh`, docs | One enforceable policy is selected and documented. |
...
| D3 | Add contract tests for mode-selection behavior and failure messaging. | `tests/core_audit/`, `tests/phase3_synthesis/` | Mode drift is automatically detected. |
| D4 | Ensure command help and docs clearly separate exploratory vs release-evidentiary invocation. | `scripts/phase3_synthesis/run_indistinguishability_test.py` help text, `governance/governance/REPRODUCIBILITY.md` | External users can execute correct mode without ambiguity. |
...
python3 -m pytest -q tests/core_audit tests/phase3_synthesis
```
...
**Addresses:** `MC-3`  
**Priority:** Medium  
**Objective:** Increase confidence in under-tested phase3_synthesis/anomaly modules (currently <20%).

### Tasks

| Task ID | Action | Target Files | Exit Criteria |
|---|---|---|---|
| E1 | Prioritize low-covered modules by release impact and add focused behavior tests. | `tests/phase2_analysis/`, `tests/phase3_synthesis/` | Targeted modules move above agreed minimum (or are explicitly exempted with rationale). |
...
| E3 | Re-run coverage baseline and publish delta in fix status report. | `core_status/ci_coverage.json`, `reports/core_audit/FIX_EXECUTION_STATUS_9.md` | Coverage improvements are measurable and traceable. |
...
| Task ID | Action | Target Files | Exit Criteria |
|---|---|---|---|
| G1 | Define acceptable dirty-release scenarios and mandatory rationale metadata. | `governance/governance/REPRODUCIBILITY.md`, `AUDIT_LOG.md` | Override usage is policy-bounded. |
| G2 | Enforce rationale capture and optional ticket linkage in pre-release script output. | `scripts/core_audit/pre_release_check.sh` | Dirty override cannot run without structured reason string. |
| G3 | Add tests for clean-tree and dirty-override gate behaviors. | `tests/core_audit/test_pre_release_contract.py` | Gate behavior is deterministic and tested. |
| G4 | Add release checklist step to summarize active diff scope before signoff. | `governance/governance/REPRODUCIBILITY.md`, release checklist docs | Final signoff includes explicit diff acknowledgment. |

### Verification Commands (post-implementation)

```bash
bash scripts/core_audit/pre_release_check.sh
ALLOW_DIRTY_RELEASE=1 DIRTY_RELEASE_REASON='planned release exception' bash scripts/core_audit/pre_release_check.sh
...
| `DOC-5` | RI-11 criteria update is codified in canonical docs and reused by core_audit/report templates. |
...
1. `bash scripts/core_audit/pre_release_check.sh`
...
- `reports/core_audit/FIX_EXECUTION_STATUS_9.md`
- `reports/core_audit/COMPREHENSIVE_AUDIT_REPORT_10.md` (next assessment-only pass)
