...
**Requested Source:** `reports/core_audit/COMPREHENSIVE_AUDIT_REPORT_100.md` (not found in workspace)  
...
|---|---|---|---|
| A1 | Build root-cause matrix for `INCONCLUSIVE` result path (`valid_scenarios=0`, collapse behavior, warning flood). | `scripts/phase2_analysis/run_sensitivity_sweep.py`, `reports/core_audit/SENSITIVITY_RESULTS.md`, `core_status/core_audit/sensitivity_sweep.json` | Matrix identifies code/config/data causes with reproducible evidence. |
| A2 | Implement deterministic remediation for failure mode(s): scenario validity logic, warning handling, gating thresholds, or dataset assumptions (as applicable). | `scripts/phase2_analysis/run_sensitivity_sweep.py`, dependent modules in `src/phase2_analysis/*` | Canonical release sweep can produce non-empty valid scenario set under policy-compliant settings. |
| A3 | Regenerate canonical release artifact and align report output. | `core_status/core_audit/sensitivity_sweep.json`, `reports/core_audit/SENSITIVITY_RESULTS.md` | Artifact fields internally consistent and reflect remediated state. |
| A4 | Keep fail-closed protections intact and add/extend contract tests. | `scripts/core_audit/pre_release_check.sh`, `scripts/verify_reproduction.sh`, `tests/core_audit/*` | Gates fail on non-ready state and pass only on truly compliant evidence. |
| A5 | Update sensitivity methodology documentation to match enforced behavior. | `governance/SENSITIVITY_ANALYSIS.md`, `governance/governance/REPRODUCIBILITY.md` | Docs match script behavior with no ambiguity. |

### Planned Verification

```bash
python3 scripts/phase2_analysis/run_sensitivity_sweep.py --dataset-id voynich_real --mode release
...
- `core_status/core_audit/sensitivity_sweep.json` shows:
  - `release_evidence_ready=true`
  - `robustness_conclusive=true`
  - `quality_gate_passed=true`
- `scripts/core_audit/pre_release_check.sh` and `scripts/verify_reproduction.sh` both pass.
...
| Task ID | Action | Target Files | Exit Criteria |
|---|---|---|---|
| B1 | Define definitive policy: strict required for release paths; non-strict allowed only for exploratory use. | `governance/governance/REPRODUCIBILITY.md`, `governance/PROVENANCE.md` | Single, explicit policy statement present and cross-referenced. |
| B2 | Enforce policy in verifier and release checks (hard fail if strict evidence absent/mismatched). | `scripts/verify_reproduction.sh`, `scripts/core_audit/pre_release_check.sh` | Release verification cannot complete on non-strict artifacts. |
| B3 | Harden CLI/help signaling for strict requirement in release context. | `scripts/phase3_synthesis/run_indistinguishability_test.py` | Help text and errors clearly separate exploratory vs release mode. |
| B4 | Add regression tests around strict policy enforcement and artifact semantics. | `tests/core_audit/*`, `tests/phase3_synthesis/test_run_indistinguishability_runner.py` | Tests detect strict-policy regressions automatically. |
...
bash scripts/verify_reproduction.sh
python3 -m pytest -q tests/core_audit/test_verify_reproduction_contract.py tests/phase3_synthesis/test_run_indistinguishability_runner.py
```
...
```bash
REQUIRE_COMPUTED=1 python3 scripts/phase3_synthesis/run_indistinguishability_test.py --preflight-only
rg -n "DATA_AVAILABILITY|BLOCKED|lost|unavailable" docs planning/core_audit reports/core_audit
...
| Task ID | Action | Target Files | Exit Criteria |
|---|---|---|---|
| D1 | Identify high-impact untested branches in `feature_discovery.py` and adjacent refinement modules. | `src/phase3_synthesis/refinement/feature_discovery.py`, `src/phase3_synthesis/refinement/*` | Prioritized branch list created with test targets. |
| D2 | Add focused unit tests for deterministic behavior, edge handling, and exception paths. | `tests/phase3_synthesis/refinement/*` | Module coverage materially improved and behavior assertions explicit. |
...
| D4 | Publish before/after coverage delta in execution status report. | `reports/core_audit/FIX_EXECUTION_STATUS_10.md` (future execution pass) | Delta is explicit and reproducible. |
...
python3 -m pytest -q tests/core_audit/test_repair_run_statuses.py
...
| Task ID | Action | Target Files | Exit Criteria |
|---|---|---|---|
| F1 | Define strict criteria for `ALLOW_DIRTY_RELEASE` usage (allowed contexts, required rationale format). | `governance/governance/REPRODUCIBILITY.md`, `AUDIT_LOG.md` | Override policy is explicit and enforceable. |
| F2 | Improve script-level validation and operator guidance for dirty-release overrides. | `scripts/core_audit/pre_release_check.sh` | Invalid or missing rationale is hard-failed with actionable guidance. |
| F3 | Add contract tests for clean-tree, dirty-tree, and override behaviors. | `tests/core_audit/test_pre_release_contract.py` | Release-hygiene behavior is regression-protected. |
| F4 | Add signoff checklist item requiring explicit diff acknowledgment at release cut. | release checklist docs, `governance/governance/REPRODUCIBILITY.md` | Final release checklist includes diff-state acknowledgment. |

### Planned Verification

```bash
bash scripts/core_audit/pre_release_check.sh
ALLOW_DIRTY_RELEASE=1 DIRTY_RELEASE_REASON='ticket: AUDIT-10 hygiene exception' bash scripts/core_audit/pre_release_check.sh
...
4. `reports/core_audit/FIX_EXECUTION_STATUS_10.md` is published with traceable evidence links to scripts/artifacts/tests.

---

## 8) Planned Deliverables (Next Execution Pass)

1. `reports/core_audit/FIX_EXECUTION_STATUS_10.md` (full remediation log and evidence).
2. Updated scripts/tests/docs per workstream outcomes.
3. Fresh verification artifacts (`core_status/core_audit/sensitivity_sweep.json`, CI coverage output, strict preflight artifacts).
4. Follow-up assessment report (`reports/core_audit/COMPREHENSIVE_AUDIT_REPORT_11.md`).
