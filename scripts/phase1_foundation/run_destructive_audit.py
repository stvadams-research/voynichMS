...
2. One that fails sensitivity phase2_analysis
...
def setup_test_infrastructure(store: MetadataStore, run_id: str, seed: int = DEFAULT_SEED):
    ...
    try:
        ...
        # Create test dataset
        store.add_dataset("audit_real", "tests/phase1_foundation/dataset")
        ...
        for page_id in test_pages:
            store.add_page(
                ...
                image_path=f"tests/phase1_foundation/dataset/{page_id}.jpg",
...
def setup_control_datasets(store: MetadataStore, run_id: str, seed: int = 42):
    ...
    session = store.Session()
    try:
        from phase1_foundation.storage.metadata import ControlDatasetRecord
...
def generate_anchors(store: MetadataStore, run_id: str, seed: int = 42):
    ...
    session = store.Session()
    try:
        from phase1_foundation.storage.metadata import AnchorRecord
