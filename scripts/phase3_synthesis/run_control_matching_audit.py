#!/usr/bin/env python3
"""
SK-H3 control matching core_audit runner.

Produces deterministic comparability artifacts without requiring full phase3_synthesis reruns.
"""
...
STATUS_PATH = PROJECT_ROOT / "core_status/phase3_synthesis/CONTROL_COMPARABILITY_STATUS.json"
DATA_AVAILABILITY_STATUS_PATH = (
    PROJECT_ROOT / "core_status/phase3_synthesis/CONTROL_COMPARABILITY_DATA_AVAILABILITY.json"
)
TURING_STATUS_PATH = PROJECT_ROOT / "core_status/phase3_synthesis/TURING_TEST_RESULTS.json"
CARDS_DIR = PROJECT_ROOT / "core_status/phase3_synthesis/control_matching_cards"
...
def run_audit(*, policy_path: Path, preflight_only: bool, seed: int, dataset_id: str) -> Dict[str, Any]:
    ...
    control_card_paths = [
        f"core_status/phase3_synthesis/control_matching_cards/{card['control_class']}.json"
    ...
    if leakage_detected:
        ...
        available_subset_allowed_claim = (
            "Available-subset comparability supports bounded structural phase4_inference only."
        )
    ...
    summary = {
        ...
        "data_availability_status_path": "core_status/phase3_synthesis/CONTROL_COMPARABILITY_DATA_AVAILABILITY.json",
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run SK-H3 control matching core_audit.")
    parser.add_argument("--policy-path", default=str(POLICY_PATH), help="Path to SK-H3 policy JSON.")
    parser.add_argument("--dataset-id", default="voynich_real", help="Dataset id used for core_audit metadata.")
    parser.add_argument("--seed", type=int, default=DEFAULT_SEED, help="Deterministic seed for core_audit artifacts.")
    parser.add_argument(
        "--preflight-only",
        action="store_true",
        help="Write deterministic core_audit artifacts without declaring full comparability closure.",
    )
...
def main() -> int:
    ...
    print(
        "[OK] control matching core_audit completed "
