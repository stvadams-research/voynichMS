...
def run_checks(policy: Dict[str, Any], *, root: Path, mode: str) -> List[str]:
    ...
    comparability_bundle = _as_dict(parsed_artifacts.get("status/synthesis/CONTROL_COMPARABILITY_STATUS.json"))
    availability_bundle = _as_dict(
        parsed_artifacts.get("status/synthesis/CONTROL_COMPARABILITY_DATA_AVAILABILITY.json")
    )
