...
if [ ! -f "${SENSITIVITY_RELEASE_STATUS_PATH}" ]; then
  ...
  echo "       python3 scripts/analysis/run_sensitivity_sweep.py --mode release --dataset-id voynich_real"
  exit 1
fi
...
if not path.exists():
    raise SystemExit(
        "[FAIL] Missing results/mechanism/anchor_coupling_confirmatory.json for SK-H1 checks."
    )
payload = json.loads(path.read_text(encoding="utf-8"))
results = payload.get("results", {})
status = results.get("status")
...
if h1_4_lane != expected_lane:
    raise SystemExit(
        "[FAIL] SK-H1.4 status/robustness mismatch: "
...
if h1_5_lane != expected_h1_5_lane:
    raise SystemExit(
        "[FAIL] SK-H1.5 status/entitlement mismatch: "
...
if h3_5_closure_lane != expected_h3_5_lane:
    raise SystemExit(
        "[FAIL] SK-H3 h3_5_closure_lane does not match status/feasibility semantics "
...
if not path.exists():
    raise SystemExit("[FAIL] Missing SK-M2 artifact: results/human/phase_7c_uncertainty.json")
...
if (
    ...
    raise SystemExit(
        "[FAIL] SK-M2 missing-folio blocking claim requires objective comparative validity failure."
    )
if m2_5_lane == "M2_5_BLOCKED" and linkage.get("objective_comparative_validity_failure") is not True:
    raise SystemExit(
        "[FAIL] SK-M2 M2_5_BLOCKED lane requires objective comparative validity failure linkage."
    )
...
if not provenance_path.exists():
    raise SystemExit("[FAIL] Missing status/audit/provenance_health_status.json")
if not sync_path.exists():
    raise SystemExit("[FAIL] Missing status/audit/provenance_register_sync_status.json")
