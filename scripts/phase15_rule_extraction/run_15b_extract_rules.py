...
def main():
    ...
    with open(REPORT_PATH, "w") as f:
        ...
        f.write("This document defines the explicit transition logic of the reconstructed mechanical generator. ")
        f.write("A third party can reproduce the manuscript's structure by following these rules.\n\n")
        ...
        f.write("- **Deterministic Reset:** Every line returns the carriage to a start position (Window 0).\n")
        f.write("- **Window Adjacency:** Scribe selection is restricted to the window exposed by the previous token's transition rule.\n")

    console.print(f"\n[green]Success! Declarative rules extracted to:[/green] {REPORT_PATH}")
    
    # 4. Save Status
    ProvenanceWriter.save_results({"num_rules_extracted": len(rules)}, project_root / "results/data/phase15_selection/rule_extraction_status.json")
