#!/usr/bin/env python3
"""Phase 19A: Line-conditioned constrained decoder evaluation."""
...
OUTPUT_PATH = PROJECT_ROOT / "results/data/phase19_alignment/line_conditioned_decoder.json"
...
def main() -> None:
    ...
    for split_name in ["train", "val", "test"]:
        ...
        split_results[split_name] = evaluate_single_method(
            model=model,
            folio_ids=folio_ids,
            method_name=f"line_conditioned_decoder/{split_name}",
            generator_fn=lambda folio_id, _split=split_name: generate_line_conditioned(
                model=model,
                folio_id=folio_id,
                stats=stats,
                seed=42,
    ...
    output = {
        "schema_version": "phase19_line_conditioned_v1",
    ...
    print("Phase 19A complete: line-conditioned decoder evaluated")
...
if __name__ == "__main__":
    with active_run(config={"seed": 42, "command": "run_19a_line_conditioned_decoder"}):
        main()
