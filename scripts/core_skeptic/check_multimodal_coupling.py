...
def run_checks(policy: dict[str, Any], *, root: Path, mode: str) -> list[str]:
    ...
    if status and coherence_policy:
        ...
        phase4_inference = results.get("phase4_inference")
        adequacy_pass = None
        ...
        if status in required_inference_by_status:
            ...
            observed = ""
            if isinstance(phase4_inference, dict):
                observed = str(phase4_inference.get("decision", ""))
            if not observed:
                errors.append(
                    f"[artifact-coherence] status `{status}` requires phase4_inference.decision="
                    f"`{expected}`, but phase4_inference.decision is missing"
                )
