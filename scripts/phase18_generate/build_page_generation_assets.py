...
OUTPUT_DIR = project_root / "results/data/phase18_generate"
...
def get_section(folio_id: str) -> str:
    folio_num = get_folio_num(folio_id)
    for section_name, (low, high) in SECTIONS.items():
        if low <= folio_num <= high:
            return section_name
    return "Other"


def get_hand(folio_id: str) -> str:
    folio_num = get_folio_num(folio_id)
    if folio_num <= 66:
        return "Hand1"
...
def parse_folio_lines(path: Path) -> dict[str, list[dict]]:
...
def build_folio_schedule(
    ...
    for folio_id in sorted(folio_lines.keys(), key=folio_sort_key):
        lines = folio_lines[folio_id]
        section = get_section(folio_id)
        hand = get_hand(folio_id)
        side = get_side(folio_id)

        folio_mode = _mode(folio_inferred_offsets.get(folio_id, []), global_mode)
...
def build_page_priors(folio_lines: dict[str, list[dict]]) -> dict:
    line_counts_global = []
    ...
    return {
        ...
        "line_count_priors": {
            "global": _distribution(line_counts_global),
            "by_section": {
                section: _distribution(values)
                for section, values in sorted(line_counts_by_section.items())
            },
            "by_side": {
                side: _distribution(values)
                for side, values in sorted(line_counts_by_side.items())
            },
        },
        "marker_priors": {
            "global": {
                "first_line": _normalize_counts(first_marker_global),
                "continuation": _normalize_counts(cont_marker_global),
            },
            "by_section": {
                section: {
                    "first_line": _normalize_counts(first_marker_by_section[section]),
                    "continuation": _normalize_counts(cont_marker_by_section[section]),
                }
                for section in sorted(line_counts_by_section.keys())
            },
            "by_side": {
                side: {
                    "first_line": _normalize_counts(first_marker_by_side[side]),
                    "continuation": _normalize_counts(cont_marker_by_side[side]),
                }
...
def main() -> None:
    ...
    folio_lines = parse_folio_lines(FOLIO_SOURCE_PATH)

    schedule_payload = build_folio_schedule(
        folio_lines=folio_lines,
        mask_inference=mask_inference,
        mask_prediction=mask_prediction,
    )
    priors_payload = build_page_priors(folio_lines=folio_lines)
